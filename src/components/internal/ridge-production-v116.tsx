...
type SourceQualityIslandTreeZone = V116Zone | "coastal-north" | "coastal-south";
...
// volcanic tarn. Every terrain carve, bank material, bed, water surface, and
// river join consumes this shared authority, so the smaller footprint exposes
// a real continuous valley floor rather than hiding it with a decorative rim.
const LAKE_RADIUS = { x: 132.4, z: 94.6 } as const;
...
function createLittoralGeologyPlacements(mobile: boolean, tier: WorldQualityTier): PlacementTuple[] {
  if (mobile || tier === "conservative") return [];
  // Candidate CE replaces the nearly continuous ring of similarly scaled
  // shoreline anchors with three unequal, erosion-led clusters. Open reaches
  // remain visible between the north shelf, inlet fan, and southern spur, so
  // these licensed forms read as exposed geology rather than a rock necklace.
  const angles = tier === "high"
    ? [-2.98, -2.82, -2.61, -1.96, -1.74, -0.92, -0.69, 0.02, 0.24, 0.91, 1.12, 1.86, 2.08, 2.36]
    : [-2.94, -2.7, -1.88, -0.79, 0.1, 0.98, 1.95, 2.22];
  const count = angles.length;
  return Array.from({ length: count }, (_, index) => {
    const angle = angles[index] + Math.sin(index * 2.17) * 0.018;
    const cluster = Math.sin(index * 1.91 + 0.4) * 0.038 + Math.sin(index * 0.57) * 0.021;
    const radius = lakeBoundaryScale(angle) * (1.016 + cluster);
    const x = LAKE_CENTER.x + Math.cos(angle) * LAKE_RADIUS.x * radius;
    const z = LAKE_CENTER.z + Math.sin(angle) * LAKE_RADIUS.z * radius;
    const waterlineVariation = Math.sin(angle * 5.0 - 0.6) * 0.22 + Math.sin(index * 1.31) * 0.12;
    return [
      0,
      2,
      x,
      LAKE_WATER_LEVEL + 0.2 + waterlineVariation,
      z,
      angle + Math.PI * 0.5,
      1,
      1,
...
function regionalVolcanicLandformRelief(x: number, z: number) {
  ...
  const warpedAxis = z * 0.017
    ...
    + flankSide * 0.47;
  const interfluves = Math.sin(warpedAxis) * (4.4 + upslope * 4.4);
  ...
  const narrowRunoff = -Math.pow(Math.max(0, runoffPhase), 8) * (4.3 + upslope * 4.2);
  const crossingRunoffPhase = Math.sin(
    z * 0.051
      - flankDistance * 0.017
      + Math.sin((z - x) * 0.011) * 0.62,
  );
  const crossingRunoff = -Math.pow(Math.max(0, crossingRunoffPhase), 12) * (1.7 + upslope * 2.35);
  ...
  const weatheredTerraces = Math.sign(terracePhase)
    ...
    * (0.82 + upslope * 1.42);
  // Two unequal mass-wasting hollows and their retained outer ribs give the
  // long Valley walls a readable kilometre-scale history. Their broad falloff
  // remains inside the already protected regional envelope, so the lake,
  // river, waterfall, and chunk joins are unchanged.
  const ellipticalRelief = (
    ...
    radiusZ: number,
    falloff = 1.8,
  ) => Math.exp(-((((x - centerX) / radiusX) ** 2 + ((z - centerZ) / radiusZ) ** 2) * falloff));
  const westernHeadwall = ellipticalRelief(-315, -760, 176, 136, 1.45) * -13.5;
  const westernRim = ellipticalRelief(-438, -748, 138, 154, 1.8) * 6.4;
  const easternHeadwall = ellipticalRelief(372, -650, 158, 126, 1.55) * -11.2;
  const easternRim = ellipticalRelief(492, -640, 132, 145, 1.9) * 5.6;
  return Math.max(
    -22.5,
    Math.min(
      17.5,
...
function valleyEscarpmentMesoEnvelope(x: number, y: number, z: number, normalY: number) {
  // Candidate BX adds an intermediate physical scale between the kilometre
  // Valley mass and its small shader grain. It operates only on the two
  // camera-facing source walls and fades before every accepted seam and water
  // authority, so the result remains one connected terrain surface.
  const longitudinal = smoothRange(-946, -908, z) * (1 - smoothRange(-458, -414, z));
  const flankDistance = Math.abs(x);
  const lateral = smoothRange(138, 196, flankDistance) * (1 - smoothRange(566, 622, flankDistance));
  const elevation = smoothRange(-42, -18, y) * (1 - smoothRange(154, 188, y));
  const steepness = 1 - Math.abs(normalY);
  const slopeAuthority = 0.42 + smoothRange(0.08, 0.68, steepness) * 0.58;
  const lakeProtection = y < LAKE_WATER_LEVEL + 34
    ? smoothRange(1.18, 1.62, lakeBoundaryDistance(x, z))
    : 1;
  const riverProtection = z >= -818 && z <= -315
    ? smoothRange(
        v116RiverHalfWidth(z) + 24,
        v116RiverHalfWidth(z) + 78,
        Math.abs(x - v116RiverCenter(z)),
      )
    : 1;
  const waterfallProtection = smoothRange(
    0.98,
    1.52,
    Math.hypot((x - 162) / 196, (z + 714) / 164),
  );
  return longitudinal
    * lateral
    * elevation
...
function valleyEscarpmentMesoRelief(x: number, z: number) {
  const side = x < 0 ? -1 : 1;
  const flankDistance = Math.abs(x);
  const wallProgress = smoothRange(160, 560, flankDistance);
  const warpedFallLine = z * 0.023
    + flankDistance * 0.008
    + Math.sin((z - side * x) * 0.0084) * 0.82
    + side * 0.73;
  const buttress = Math.sin(warpedFallLine) * (3.2 + wallProgress * 4.6);
  const secondaryButtress = Math.sin(
    z * 0.047 - flankDistance * 0.016 + Math.sin((z + x) * 0.011) * 0.54,
  ) * (0.9 + wallProgress * 1.8);
  const scarpPhase = Math.sin(
    z * 0.069 + flankDistance * 0.021 + Math.sin(z * 0.016 - side * 0.4) * 0.76,
  );
  const brokenScarps = Math.sign(scarpPhase)
    * Math.pow(Math.abs(scarpPhase), 9)
    * (0.8 + wallProgress * 1.8);
  const gullyPhase = Math.sin(
    z * 0.034 + flankDistance * 0.028 + Math.sin(z * 0.012) * 1.14 + side * 1.3,
  );
  const branchingGully = -Math.pow(Math.max(0, gullyPhase), 13) * (3.4 + wallProgress * 5.8);
  const collapseCenterX = side < 0 ? -342 : 386;
  const collapseCenterZ = side < 0 ? -812 : -672;
  const collapse = -Math.exp(-(
    ((x - collapseCenterX) / 138) ** 2
      + ((z - collapseCenterZ) / 116) ** 2
  ) * 1.42) * 8.6;
  const retainingRib = Math.exp(-(
    ((x - (collapseCenterX + side * 102)) / 104) ** 2
      + ((z - (collapseCenterZ + 18)) / 138) ** 2
  ) * 1.72) * 5.1;
  return Math.max(
    -16.5,
    Math.min(13.5, buttress + secondaryButtress + brokenScarps + branchingGully + collapse + retainingRib),
  );
}
...
function createWeldedTerrainSourceGeometry(source: Mesh) {
  const transformed = source.geometry.clone();
  transformed.applyMatrix4(source.matrixWorld);
  const positionOnly = new BufferGeometry();
  positionOnly.setAttribute("position", transformed.getAttribute("position").clone());
  if (transformed.getIndex()) positionOnly.setIndex(transformed.getIndex()!.clone());
  transformed.dispose();

  // This source is rendered with world-projected material sampling. Preserve
  // its exact positions and triangles, but collapse duplicated UV-island
  // vertices into one geometric heightfield before any authored deformation.
  // A generated planar UV keeps Three's map sampler valid; it is not used as
  // the visible material-scale authority.
  const welded = mergeVertices(positionOnly, 0.0001);
  positionOnly.dispose();
  welded.computeBoundingBox();
  const bounds = welded.boundingBox;
  const positions = welded.getAttribute("position");
  const width = Math.max(0.001, (bounds?.max.x ?? 1) - (bounds?.min.x ?? 0));
  const depth = Math.max(0.001, (bounds?.max.z ?? 1) - (bounds?.min.z ?? 0));
  const uvs = new Float32Array(positions.count * 2);
  for (let vertex = 0; vertex < positions.count; vertex += 1) {
    uvs[vertex * 2] = (positions.getX(vertex) - (bounds?.min.x ?? 0)) / width;
    uvs[vertex * 2 + 1] = (positions.getZ(vertex) - (bounds?.min.z ?? 0)) / depth;
  }
  welded.setAttribute("uv", new Float32BufferAttribute(uvs, 2));
  welded.computeVertexNormals();
  welded.userData.sourceWeld = {
    method: "position-only weld before authored source-surface deformation",
    preservesSourceTriangles: true,
    sourceVertices: source.geometry.getAttribute("position").count,
    weldedVertices: welded.getAttribute("position").count,
    worldProjectedUvAuthority: true,
  };
  return welded;
}
...
function reconcileCoincidentTerrainNormals(geometry: BufferGeometry) {
  const positions = geometry.getAttribute("position");
  const index = geometry.getIndex();
  if (!index || !positions.count) {
    geometry.computeVertexNormals();
    return { coincidentGroups: 0, reconciledVertices: 0, triangles: 0 };
  }

  // The terrain source intentionally preserves UV seams, so vertices that are
  // geometrically identical can still be split into separate index islands.
  // Three's default normal pass cannot see across those islands; slope-driven
  // color and light then expose the source triangulation as giant wedges. Sum
  // area-weighted face normals by quantized world position and write the same
  // continuous normal back to every coincident vertex without changing UVs,
  // topology, silhouette, water boundaries, or the licensed source texture.
  const precision = 10_000;
  const keys = new Array<string>(positions.count);
  const groups = new Map<string, { count: number; x: number; y: number; z: number }>();
  const keyFor = (vertex: number) => {
    const existing = keys[vertex];
  ...
  for (let vertex = 0; vertex < positions.count; vertex += 1) keyFor(vertex);
  for (let offset = 0; offset < index.count; offset += 3) {
    ...
    [a, b, c].forEach((vertex) => {
      const group = groups.get(keys[vertex]);
      if (!group) return;
      group.x += faceX;
      group.y += faceY;
      group.z += faceZ;
  ...
  groups.forEach((group) => {
    if (group.count > 1) {
      coincidentGroups += 1;
      reconciledVertices += group.count;
    }
  });
  const normals = new Float32BufferAttribute(new Float32Array(positions.count * 3), 3);
  for (let vertex = 0; vertex < positions.count; vertex += 1) {
    const group = groups.get(keys[vertex]);
    const length = group ? Math.hypot(group.x, group.y, group.z) : 0;
    normals.setXYZ(
      vertex,
      length > 0 && group ? group.x / length : 0,
      length > 0 && group ? group.y / length : 1,
      length > 0 && group ? group.z / length : 0,
    );
  }
  geometry.setAttribute("normal", normals);
  return {
    coincidentGroups,
    method: "area-weighted normal reconciliation across coincident UV-split source vertices",
    preservesTopology: true,
    reconciledVertices,
    triangles: index.count / 3,
    waterBoundariesProtected: true,
  };
}
...
function subdivideRegionalVolcanicTerrainGeometry(source: BufferGeometry) {
  return subdivideSelectedTerrainGeometry(
    ...
    (positions, a, b, c) => {
      ...
      const steepness = normalLength > 0 ? 1 - Math.abs(normalY / normalLength) : 0;
      const longestEdge = Math.max(
        Math.hypot(abX, abY, abZ),
        Math.hypot(cx - bx, cy - by, cz - bz),
        Math.hypot(acX, acY, acZ),
      );
      const cameraFacingWall = Math.abs(centerX) > 118
        && Math.abs(centerX) < 575
        && centerY > -42
        && centerY < 172
        && centerZ > -948
        && centerZ < -405;
      return cameraFacingWall && steepness > 0.2 && longestEdge > 3.8;
    },
  );
}
...
function relaxAlpineSummitHeightfield(geometry: BufferGeometry, iterations: number) {
  const positions = geometry.getAttribute("position");
  const index = geometry.getIndex();
  if (!index || iterations <= 0) return { adjustedVertices: 0, iterations: 0, maximumAdjustmentMeters: 0 };
  const selected = new Uint8Array(positions.count);
  const maximumNeighbours = 12;
  const neighbourCounts = new Uint8Array(positions.count);
  const neighbours = new Int32Array(positions.count * maximumNeighbours);
  neighbours.fill(-1);
  const connect = (left: number, right: number) => {
    const count = neighbourCounts[left];
    const offset = left * maximumNeighbours;
    for (let index = 0; index < count; index += 1) {
      if (neighbours[offset + index] === right) return;
    }
    if (count >= maximumNeighbours) return;
    neighbours[offset + count] = right;
    neighbourCounts[left] = count + 1;
  };
  for (let offset = 0; offset < index.count; offset += 3) {
    const a = index.getX(offset);
    const b = index.getX(offset + 1);
  ...
  for (let vertex = 0; vertex < positions.count; vertex += 1) {
    ...
    if (x > 330 && x < 900 && y > 102 && z > -1608 && z < -1024) selected[vertex] = 1;
  }
  let adjustedVertices = 0;
  let maximumAdjustmentMeters = 0;
  for (let iteration = 0; iteration < iterations; iteration += 1) {
    const nextHeights = new Float32Array(positions.count);
    for (let vertex = 0; vertex < positions.count; vertex += 1) nextHeights[vertex] = positions.getY(vertex);
    for (let vertex = 0; vertex < positions.count; vertex += 1) {
      if (!selected[vertex]) continue;
      const connected = neighbourCounts[vertex];
      if (!connected) continue;
      let total = 0;
      const offset = vertex * maximumNeighbours;
      for (let neighbour = 0; neighbour < connected; neighbour += 1) {
        total += positions.getY(neighbours[offset + neighbour]);
      }
      const average = total / connected;
      const y = positions.getY(vertex);
      const adjustment = Math.max(-5.5, Math.min(5.5, (average - y) * 0.56));
      nextHeights[vertex] = y + adjustment;
      maximumAdjustmentMeters = Math.max(maximumAdjustmentMeters, Math.abs(adjustment));
      adjustedVertices += 1;
    }
    for (let vertex = 0; vertex < positions.count; vertex += 1) {
      if (selected[vertex]) positions.setY(vertex, nextHeights[vertex]);
    }
  }
  return {
    adjustedVertices,
    ...
    method: "bounded two-ring heightfield relaxation after amphitheater incision",
...
function createAlpineGeologyTerrainGeometry(source: Mesh, detail: "compact" | "detailed") {
  ...
  const crownSubdivision: Array<Record<string, unknown>> = [];
  const crownSubdivisionPasses = detail === "detailed" ? 2 : 0;
  for (let pass = 0; pass < crownSubdivisionPasses; pass += 1) {
    const previous = geometry;
    geometry = subdivideAlpineCrownTerrainGeometry(previous);
    crownSubdivision.push(geometry.userData.alpineCrownSubdivision ?? {});
    previous.dispose();
  }
  ...
  let candidateCeWeatheredVertices = 0;
  let candidateCeMaximumRelief = 0;
  ...
  for (let index = 0; index < positions.count; index += 1) {
    ...
    const candidateCeWeathering = (
      Math.sin(x * 0.046 - z * 0.031 + Math.sin(y * 0.036) * 0.74) * 0.72
      + Math.sin(x * 0.091 + z * 0.022 - 2.1) * 0.34
      - Math.pow(Math.max(0, Math.cos(x * 0.037 + z * 0.028)), 20) * 1.02
    ) * envelope * (0.24 + steepness * 0.76);
    ...
    if (horizontalNormalLength > 0.001) {
      const horizontalRelief = relief * (0.18 + steepness * 0.28)
        // Large normal-directed offsets made the eroded crown fold into blocky
        // overhangs. Keep the amphitheater authority primarily vertical, as a
        // heightfield, with only a restrained lateral rock response.
        + summitMacroRelief * (0.012 + steepness * 0.02);
    ...
    if (Math.abs(candidateCeWeathering) > 0.001) {
      candidateCeWeatheredVertices += 1;
      candidateCeMaximumRelief = Math.max(candidateCeMaximumRelief, Math.abs(candidateCeWeathering));
    }
  ...
  // The former five-pass relaxation rounded the new paired crown back into a
  // soft dome. Two bounded passes retain continuous normals while preserving
  // the physical scarps and drainage hierarchy introduced above.
  const summitSurfaceRelaxation = relaxAlpineSummitHeightfield(geometry, detail === "detailed" ? 2 : 1);
  positions.needsUpdate = true;
  if (geometry.getAttribute("uv") && !geometry.getAttribute("uv1")) {
    geometry.setAttribute("uv1", geometry.getAttribute("uv").clone());
  }
  const surfaceNormalReconciliation = reconcileCoincidentTerrainNormals(geometry);
  ...
  geometry.userData.alpineGeology = {
    ...
    candidateCeMesoWeathering: {
      adjustedVertices: candidateCeWeatheredVertices,
      detachedGeometry: false,
      maximumReliefMeters: candidateCeMaximumRelief,
      method: "bounded dual-frequency frost-weathering and narrow subtractive fracture runs on the connected Alpine source surface",
      valleyBoundaryProtected: true,
      worldBoundsProtected: true,
    },
...
function createRidgeErosionTerrainGeometry(source: Mesh) {
  ...
  for (let index = 0; index < positions.count; index += 1) {
    ...
    const candidateCeWeathering = (
      Math.sin(x * 0.084 - z * 0.026 + Math.sin(z * 0.019) * 0.82) * 0.68
      + Math.sin(x * 0.037 + z * 0.059 + 1.6) * 0.42
      - Math.pow(Math.max(0, Math.cos(x * 0.052 - z * 0.017)), 18) * 1.16
    ) * envelope * (0.28 + steepness * 0.72);
    const combinedRelief = relief + candidateCeWeathering;
    const horizontalNormalLength = Math.hypot(normalX, normalZ);
    if (horizontalNormalLength > 0.001) {
      const horizontalRelief = combinedRelief * (0.1 + steepness * 0.16);
      positions.setX(index, x + (normalX / horizontalNormalLength) * horizontalRelief);
      positions.setZ(index, z + (normalZ / horizontalNormalLength) * horizontalRelief);
    }
    positions.setY(index, y + combinedRelief * (0.94 + steepness * 0.08));
    adjustedVertices += 1;
    maximumIncision = Math.max(maximumIncision, -combinedRelief);
    maximumUplift = Math.max(maximumUplift, combinedRelief);
    if (Math.abs(candidateCeWeathering) > 0.001) {
      candidateCeWeatheredVertices += 1;
      candidateCeMaximumRelief = Math.max(candidateCeMaximumRelief, Math.abs(candidateCeWeathering));
    }
  ...
  geometry.userData.ridgeErosion = {
    ...
    candidateCeMesoWeathering: {
      ...
      detachedGeometry: false,
      maximumReliefMeters: candidateCeMaximumRelief,
      method: "bounded dual-frequency colluvial breakup plus narrow subtractive rills on the connected Ridge source surface",
      ridgeValleyBoundaryProtected: true,
      worldBoundsProtected: true,
    },
  };
...
function createIntegratedWatershedTerrainGeometry(
  ...
  let valleyEscarpmentMesoVertices = 0;
  let maximumValleyEscarpmentMesoIncision = 0;
  let maximumValleyEscarpmentMesoUplift = 0;
  ...
  for (let index = 0; index < positions.count; index += 1) {
    ...
    const escarpmentMesoEnvelope = valleyEscarpmentMesoEnvelope(x, y, z, normalY);
    if (escarpmentMesoEnvelope > 0.001) {
      const relief = valleyEscarpmentMesoRelief(x, z) * escarpmentMesoEnvelope;
      const steepness = 1 - Math.abs(normalY);
      const horizontalNormalLength = Math.hypot(normalX, normalZ);
      if (horizontalNormalLength > 0.001) {
        const horizontalRelief = relief * (0.08 + steepness * 0.17);
        positions.setX(index, positions.getX(index) + (normalX / horizontalNormalLength) * horizontalRelief);
        positions.setZ(index, positions.getZ(index) + (normalZ / horizontalNormalLength) * horizontalRelief);
      }
      nextY += relief * (0.9 + steepness * 0.1);
      valleyEscarpmentMesoVertices += 1;
      maximumValleyEscarpmentMesoIncision = Math.max(maximumValleyEscarpmentMesoIncision, -relief);
      maximumValleyEscarpmentMesoUplift = Math.max(maximumValleyEscarpmentMesoUplift, relief);
    }
    ...
    // Candidate CE adds restrained physical breakup to the broad remaining
    // Valley surfaces. It fades completely at every decoded world boundary
    // and around the lake, river, and waterfall, so it cannot disturb the
    // accepted continuous hydrology or manufacture a detached cover shell.
    const candidateCeBoundaryEnvelope = smoothRange(-958, -924, z)
      * (1 - smoothRange(-366, -326, z))
      * smoothRange(-888, -840, x)
      * (1 - smoothRange(840, 888, x));
    const candidateCeLakeProtection = smoothRange(1.18, 1.48, lakeBoundaryDistance(x, z));
    const candidateCeRiverProtection = z >= -820 && z <= -304
      ? smoothRange(v116RiverHalfWidth(z) * 1.8, v116RiverHalfWidth(z) * 4.2, Math.abs(x - v116RiverCenter(z)))
      : 1;
    const candidateCeWaterfallProtection = smoothRange(
      0.92,
      1.42,
      Math.hypot((x - 154) / 128, (z + 714) / 104),
    );
    const candidateCeSlope = 1 - Math.abs(normalY);
    const candidateCeEnvelope = candidateCeBoundaryEnvelope
      * candidateCeLakeProtection
      * candidateCeRiverProtection
      * candidateCeWaterfallProtection
      * (0.22 + smoothRange(0.035, 0.66, candidateCeSlope) * 0.78);
    if (candidateCeEnvelope > 0.001) {
      const candidateCeWeathering = (
        Math.sin(x * 0.058 - z * 0.019 + Math.sin(z * 0.014) * 0.92) * 0.86
        + Math.sin(x * 0.103 + z * 0.037 + 0.8) * 0.41
        - Math.pow(Math.max(0, Math.cos(x * 0.044 - z * 0.029 + 1.4)), 18) * 1.34
      ) * candidateCeEnvelope;
      nextY += candidateCeWeathering;
      candidateCeWeatheredVertices += 1;
      candidateCeMaximumRelief = Math.max(candidateCeMaximumRelief, Math.abs(candidateCeWeathering));
    }
  ...
  let surfaceNormalReconciliation = reconcileCoincidentTerrainNormals(geometry);
  ...
  surfaceNormalReconciliation = reconcileCoincidentTerrainNormals(geometry);
  geometry.computeBoundingBox();
  geometry.computeBoundingSphere();
  ...
  geometry.userData.watershedIntegration = {
    ...
    renderedTriangles: (geometry.index?.count ?? geometry.getAttribute("position").count) / 3,
    sourceWeld,
    riverCorridor: {
    ...
    valleyEscarpmentMesoRelief: {
      adjustedVertices: valleyEscarpmentMesoVertices,
      detachedGeometry: false,
      lakeRiverAndWaterfallProtected: true,
      maximumIncisionMeters: maximumValleyEscarpmentMesoIncision,
      maximumUpliftMeters: maximumValleyEscarpmentMesoUplift,
      method: "paired camera-facing collapse hollows, retaining ribs, branching gullies, and broken scarps on the active Valley source surface",
      ridgeAndAlpineBoundariesProtected: true,
    },
    surfaceNormalReconciliation,
    ...
    candidateCeMesoWeathering: {
      ...
      lakeRiverAndWaterfallProtected: true,
      maximumReliefMeters: candidateCeMaximumRelief,
      method: "bounded dual-frequency colluvial breakup and narrow subtractive rills across remaining connected Valley surfaces",
      ridgeAndAlpineBoundariesProtected: true,
    },
...
function DetailedTerrainChunk({ connectedCoast = false, shadows, tier, zone }: {
  ...
  const southernCoastalBoundary = useMemo(() => {
    if (!connectedCoast || zone !== "valley") return [];
    gltf.scene.updateMatrixWorld(true);
    const source = gltf.scene.getObjectByName(DETAILED_TERRAIN_OBJECTS.valley);
    return source instanceof Mesh ? extractCoastalBoundarySamples(source, -310) : [];
  ...
  const southernCoastalPlacements = useMemo(() => {
    if (!southernCoastalBoundary.length) return [];
    const shoulder = createCoastalShoulderGeometry(
      false,
      tier,
      southernCoastalBoundary,
      [],
      undefined,
      "valley",
    );
  ...
  const connectedValleyCoastGeometry = useMemo(() => {
    if (!terminalChunkGeometry || !southernCoastalBoundary.length) return null;
    const shoulder = createCoastalShoulderGeometry(
      false,
      tier,
      southernCoastalBoundary,
    ...
    const result = createConnectedCoastalTerrainGeometry(
      terminalChunkGeometry,
      shoulder,
      southernCoastalBoundary[0]?.x ?? -310,
      "valley",
    );
    shoulder.dispose();
    return result;
  }, [southernCoastalBoundary, terminalChunkGeometry, tier]);
  ...
  useEffect(() => {
    ...
    if (connectedValleyCoastGeometry) {
      host.__MADAGIN_COASTAL_SHOULDER_V116__ = {
        ...(host.__MADAGIN_COASTAL_SHOULDER_V116__ ?? {}),
        candidate: "BZ",
        coastlineAuthority: "shared by connected terrain, shoal depth, breaker displacement, foam, and backwash",
        exposedCoastalVoidClosed: true,
        southernExtension: connectedValleyCoastGeometry.userData.coastalExtension ?? null,
        southernBoundarySamples: southernCoastalBoundary.length,
        southernBoundaryZ: [
          southernCoastalBoundary[0]?.z ?? null,
          southernCoastalBoundary[southernCoastalBoundary.length - 1]?.z ?? null,
        ],
      };
      document.documentElement.dataset.madaginCoastalShoulderV116 = JSON.stringify(host.__MADAGIN_COASTAL_SHOULDER_V116__);
      dispatchStage(1, "summit-southern-coastal-void-closed", "valley");
    }
    return () => {
      alpineGeometry?.dispose();
      connectedGeometry?.dispose();
      connectedValleyCoastGeometry?.dispose();
    ...
    <>
      ...
      {coastalPlacements.length ? (
        <Suspense fallback={null}>
          <CoastalEcology placements={coastalPlacements} shadows={shadows} zone="coastal-north" />
        </Suspense>
      ) : null}
    </>
  ) : connectedValleyCoastGeometry ? (
    <>
      <mesh
        geometry={connectedValleyCoastGeometry}
        material={material}
        name="Madagin Candidate BZ single-indexed Valley-to-ocean coastal terrain"
        receiveShadow
      />
      {southernCoastalPlacements.length ? (
        <Suspense fallback={null}>
          <CoastalEcology placements={southernCoastalPlacements} shadows={shadows} zone="coastal-south" />
        </Suspense>
      ) : null}
    </>
  ) : terminalChunkGeometry ? (
...
// Candidate BZ gives terrain and ocean one coastline authority. The previous
// shoulder narrowed to the Ridge boundary while the water continued south,
// leaving a sky-colored wedge between the terminal Valley and the surf. Keep
// the low-frequency coves broad enough to read as geography at the Ocean view.
function coastlineAuthorityX(z: number) {
  return -690
    + Math.sin(z * 0.012 + 0.8) * 18
    + Math.sin(z * 0.029 - 1.3) * 7.5
    + Math.sin(z * 0.061 + 0.35) * 2.8;
}
...
function createCoastalShoulderGeometry(
  ...
  for (let row = 0; row <= rows; row += 1) {
    ...
    const outerX = coastlineAuthorityX(z) + 2.6;
  ...
  geometry.userData.coastalSpan = {
    ...
    span,
    triangles: rows * columns * 2,
    zRangeMeters: [boundarySamples[0]?.z ?? -315, boundarySamples[boundarySamples.length - 1]?.z ?? 285],
  };
  return geometry;
}
...
function createCoastalPlacements(geometry: BufferGeometry, mobile: boolean, tier: WorldQualityTier) {
  const positions = geometry.getAttribute("position");
  const normals = geometry.getAttribute("normal");
  const structuralSpan = geometry.userData.coastalSpan as { columns?: number; span?: "ridge" | "valley" } | undefined;
  const columns = structuralSpan?.columns
    ?? (mobile ? 24 : tier === "high" ? 60 : tier === "balanced" ? 48 : 32);
  const rows = positions.count / (columns + 1) - 1;
  const southernExtension = structuralSpan?.span === "valley";
  const limit = mobile
  ...
  // Candidate BY advanced through this grid until it reached a cap, leaving
  // conspicuous rows and front-loading the ecology into one end of the coast.
  // Rank a jittered candidate set across the whole connected landform instead:
  // the bounded result keeps the same rendering budget while reading as
  // irregular wind-pruned succession rather than a plantation.
  for (let row = southernExtension ? 2 : mobile ? 4 : 6; row < rows - 2; row += 1) {
    for (let column = 2 + (row % 2); column < columns - 1; column += 2) {
      const index = row * (columns + 1) + column;
      const x = positions.getX(index);
      const y = positions.getY(index);
      const z = positions.getZ(index);
      ...
      const nextColumn = Math.min(columns, column + 1);
      const nextRow = Math.min(rows, row + 1);
      const cellX = Math.max(0.8, Math.abs(positions.getX(row * (columns + 1) + nextColumn) - x));
      ...
      const scale = (layer === 0 ? 0.55 : 0.36) + stature * (layer === 0 ? 0.48 : 0.29);
      const placement: PlacementTuple = [
        family,
        layer,
        x + jitterX,
        y + jitterY + 0.035,
      ...
      candidates.push({ placement, rank: seededUnit(seed + 83) });
    }
  }
  return candidates
...
function CoastalEcology({ placements, shadows, zone = "coastal-north" }: {
  placements: PlacementTuple[];
  shadows: boolean;
  zone?: Extract<SourceQualityIslandTreeZone, "coastal-north" | "coastal-south">;
}) {
  ...
  const sourceQualitySet = useMemo(() => new Set(sourceQualityPlacements), [sourceQualityPlacements]);
  const batches = useMemo(() => {
    const grouped = new Map<string, PlacementTuple[]>();
    placements.filter((placement) => !sourceQualitySet.has(placement)).forEach((placement) => {
      const key = COASTAL_FAMILIES[placement[0] % COASTAL_FAMILIES.length];
      grouped.set(key, [...(grouped.get(key) ?? []), placement]);
    });
    return grouped;
  }, [placements, sourceQualitySet]);
  useEffect(() => {
  ...
  return (
    <group name={`Madagin Candidate CD grounded wind-pruned ${zone} ecology · ${placements.length}`}>
      ...
      {sourceQualityPlacements.length ? (
        <Suspense fallback={null}>
          <SourceQualityIslandTree01Anchors
            placements={sourceQualityPlacements}
            shadows={shadows}
            zone={zone}
          />
        </Suspense>
...
function createConnectedCoastalTerrainGeometry(
  terrain: BufferGeometry,
  shoulder: BufferGeometry,
  boundaryX: number,
  span: "ridge" | "valley",
) {
  const terrainSurface = geometrySurfaceForMerge(terrain);
  const shoulderSurface = geometrySurfaceForMerge(shoulder);
  const removedBoundaryWallTriangles = removeCoplanarBoundaryWall(terrainSurface, "x", boundaryX);
  const merged = mergeGeometries([terrainSurface, shoulderSurface], false);
  terrainSurface.dispose();
  ...
  const uvs: number[] = [];
  for (let index = 0; index < positions.count; index += 1) {
    uvs.push((positions.getX(index) + 690) / 820, (285 - positions.getZ(index)) / 1265);
  }
  const uv = new Float32BufferAttribute(uvs, 2);
  connected.setAttribute("uv", uv);
  connected.setAttribute("uv1", uv.clone());
  ...
  connected.computeBoundingBox();
  connected.computeBoundingSphere();
  connected.name = `Madagin Candidate BZ connected ${span} coastal terrain`;
  connected.userData.coastalExtension = {
    boundaryX,
    detachedGeometry: false,
    joinedVertices: positions.count,
    method: "single-indexed terrain-to-shore remesh using the shared ocean coastline authority",
    removedBoundaryWallTriangles,
    span,
...
function variantMaterials(source: Material | Material[]) {
  ...
  const result = originals.map((original) => {
    ...
    const material = new MeshStandardMaterial({
      color: foliage ? "#c6d0be" : "#78604d",
      emissive: foliage ? "#29482d" : "#070503",
      emissiveIntensity: foliage ? 0.28 : 0.015,
      envMapIntensity: foliage ? 0.36 : 0.18,
      metalness: 0,
...
function volumetricCrownLobeCount(family: string, placement: PlacementTuple, mobile: boolean) {
  if (mobile || placement[1] > 1 || !VOLUMETRIC_CROWN_FAMILIES.has(family)) return 1;
  if (placement[1] === 1) return 2;
  return family === "alpine_conifer" || family === "ridge_wind_form" ? 2 : 4;
}
...
function InstancedSpeciesBatch({ mobile = false, part, placements, shadows }: {
  ...
  useLayoutEffect(() => {
    ...
    instances.forEach(({ lobe, lobeCount, placement, placementIndex }, index) => {
      ...
      const lobeAngle = placement[5]
        + lobe * (Math.PI * 2 / Math.max(1, lobeCount))
        + (signature % 7) * 0.11;
      const lateralOffset = lobe === 0 ? 0 : placement[6] * (lobeCount === 2 ? 1.08 : lobe === 1 ? 1.18 : 0.96);
      const verticalOffset = lobe === 0
        ? 0
      ...
      const foliagePalette = ["#78906f", "#698663", "#828665", "#638069", "#89865f", "#718978", "#7c7d58", "#68866f"];
...
function RidgeBasaltField({ placements, shadows, zone }: {
  ...
  const selected = useMemo(
    () => placements
      .filter((placement, index) => placement[1] === 2 && index % (zone === "ridge" ? 17 : zone === "alpine" ? 11 : 23) === 0)
      .slice(0, zone === "ridge" ? 92 : zone === "alpine" ? 110 : 78),
    [placements, zone],
  );
...
function prepareSourceQualityPachira(scene: Object3D) {
  ...
  scene.traverse((child) => {
    ...
    const materials = originals.map((original) => {
      ...
      material.color.set(foliage ? "#ccd7c5" : "#8d694e");
      material.emissive.set(foliage ? "#234527" : "#0c0805");
      material.emissiveIntensity = foliage ? 0.27 : 0.025;
      material.emissiveMap = null;
      material.envMapIntensity = foliage ? 0.38 : 0.24;
      material.metalness = 0;
...
function selectSourceQualityPachiraPlacements(
  ...
  const limit = tier === "high" ? 28 : zone === "lake" ? 16 : zone === "valley" ? 24 : 20;
  return placements
...
function InstancedSourceQualityPachira({ part, placements, shadows, zone }: {
  ...
  useLayoutEffect(() => {
    ...
    placements.forEach((placement, index) => {
      ...
      const age = 0.58 + (signature % 13) * 0.052;
      const crownWidth = 0.72 + ((signature * 5) % 15) * 0.036;
      const crownDepth = 0.76 + ((signature * 7) % 13) * 0.041;
      dummy.position.set(
        placement[2],
      ...
      matrix.multiplyMatrices(dummy.matrix, part.matrixWorld);
      mesh.setMatrixAt(index, matrix);
      const foliagePalette = ["#78956d", "#668761", "#84976a", "#5f8065", "#8b8c5d"];
...
function InstancedSourceQualityIslandTree01({ part, placements, shadows, zone }: {
  ...
  useLayoutEffect(() => {
    ...
    placements.forEach((placement, index) => {
      ...
      const coastal = zone === "coastal-north" || zone === "coastal-south";
      const baseScale = (coastal ? 1.08 : zone === "valley" ? 0.88 : zone === "lake" ? 0.74 : 0.8) * layerScale;
...
function SourceQualityIslandTree01Anchors({ placements, shadows, zone }: {
  ...
  useEffect(() => {
    ...
    dispatchStage(
      3,
      `${zone}-source-quality-island-tree-01-ready`,
      zone === "coastal-north" ? "ridge" : zone === "coastal-south" ? "valley" : zone,
    );
    return () => parts.forEach((part) => {
      (Array.isArray(part.material) ? part.material : [part.material]).forEach((material) => material.dispose());
    });
...
function prepareSourceQualityGeology(scene: Object3D) {
  scene.updateMatrixWorld(true);
  const parts: SourceQualityGeologyPart[] = [];
  scene.traverse((child) => {
    if (!(child instanceof Mesh)) return;
    const originals = Array.isArray(child.material) ? child.material : [child.material];
    const materials = originals.map((original) => {
      const material = (original as MeshStandardMaterial).clone();
      material.color.multiply(new Color("#c8cabe"));
      material.emissive.set("#18231a");
    ...
    const matrixWorld = child.matrixWorld.clone();
    // The source scene arranges its seven rocks as a contact sheet. Each mesh
    // becomes an independent grounded form at runtime, so retain authored
    // rotation/scale but remove the sheet-layout translation.
    matrixWorld.setPosition(0, 0, 0);
    parts.push({
      family: child.name,
      geometry: child.geometry,
      material: Array.isArray(child.material) ? materials : materials[0],
      matrixWorld,
      sourceKey: child.name,
    });
  });
  return parts;
}

function sourceQualityGeologySignature(placement: PlacementTuple) {
  return Math.abs(Math.round(
    placement[2] * 1.17
      + placement[3] * 2.31
      + placement[4] * 0.83
...
function selectSourceQualityGeologyPlacements(
  instances: PlacementTuple[],
  mobile: boolean,
  tier: WorldQualityTier,
  zone: V116Zone,
  ...
  const candidates = zone === "lake"
    ? grounded.filter((placement) => {
      const lakeDistance = lakeBoundaryDistance(placement[2], placement[4]);
      return lakeDistance >= 0.84 && lakeDistance <= 1.28;
    })
    : grounded.filter((placement) => {
      if (zone === "ridge") return placement[3] >= -12 && placement[3] <= 38;
...
function InstancedSourceQualityGeology({ part, placements, shadows, zone }: {
  part: SourceQualityGeologyPart;
  placements: PlacementTuple[];
  shadows: boolean;
  zone: V116Zone;
}) {
  ...
  useLayoutEffect(() => {
    ...
    placements.forEach((placement, index) => {
      ...
      dummy.rotation.set(
        ...
        (((signature * 5) % 11) - 5) * 0.01,
      );
      dummy.scale.set(
        size * (0.82 + ((signature * 3) % 9) * 0.05),
        size * (0.68 + ((signature * 7) % 8) * 0.045),
        size * (0.86 + ((signature * 11) % 7) * 0.055),
      );
      dummy.updateMatrix();
      matrix.multiplyMatrices(dummy.matrix, part.matrixWorld);
      mesh.setMatrixAt(index, matrix);
...
function SourceQualityGeologyAnchors({ placements, shadows, zone }: {
  placements: PlacementTuple[];
  shadows: boolean;
  zone: V116Zone;
}) {
  ...
  const bySource = useMemo(() => {
    ...
    placements.forEach((placement, index) => {
      const part = parts[(sourceQualityGeologySignature(placement) + index) % parts.length];
      if (!part) return;
      result.set(part.sourceKey, [...(result.get(part.sourceKey) ?? []), placement]);
    });
    return result;
  }, [parts, placements]);
  useEffect(() => {
    const host = window as Window & { __MADAGIN_SOURCE_GEOLOGY_BS__?: Record<string, unknown> };
    host.__MADAGIN_SOURCE_GEOLOGY_BS__ = {
      ...(host.__MADAGIN_SOURCE_GEOLOGY_BS__ ?? {}),
      [zone]: {
        forms: parts.length,
        placements: placements.length,
        source: SOURCE_QUALITY_GEOLOGY_URL,
      },
    };
    document.documentElement.dataset.madaginSourceGeologyBs = JSON.stringify(host.__MADAGIN_SOURCE_GEOLOGY_BS__);
    dispatchStage(3, `${zone}-source-quality-geology-anchors-ready`, zone);
    return () => parts.forEach((part) => {
      (Array.isArray(part.material) ? part.material : [part.material]).forEach((material) => material.dispose());
    });
  }, [parts, placements.length, zone]);
  return (
    <group name={`Madagin Candidate BS ${zone} source-quality mossy geology · ${placements.length}`}>
      {parts.flatMap((part, index) => {
        const sourcePlacements = bySource.get(part.sourceKey) ?? [];
        return sourcePlacements.length ? (
          <InstancedSourceQualityGeology
            key={`source-geology-${zone}-${part.sourceKey}-${index}`}
            part={part}
            placements={sourcePlacements}
            shadows={shadows}
            zone={zone}
...
function prepareDetailedVegetation(scene: Object3D, mode: DetailedVegetationMode) {
  ...
  scene.traverse((child) => {
    ...
    const materials = originals.map((original) => {
      ...
      material.depthWrite = true;
      material.emissive.set(foliage ? "#294b2d" : "#080604");
      material.emissiveIntensity = foliage ? 0.32 : 0.018;
      material.emissiveMap = null;
      material.envMapIntensity = foliage ? 0.36 : 0.18;
      material.metalness = 0;
...
function isMultiSlopeSuccessionPlacement(placement: PlacementTuple, zone: V116Zone) {
  ...
  if (zone === "alpine") return placement[1] === 1
    && placement[3] >= 20
    && placement[3] <= 205;
  return false;
}
...
function InstancedDetailedVegetation({ mode, part, placements, shadows, zone }: {
  ...
  useLayoutEffect(() => {
    ...
    placements.forEach((placement, index) => {
      ...
      matrix.multiplyMatrices(dummy.matrix, part.matrixWorld);
      mesh.setMatrixAt(index, matrix);
      const foliagePalette = ["#829776", "#698966", "#8a8762", "#66816a", "#87946e", "#747f60", "#6b8973", "#91895f"];
      const barkPalette = ["#80624c", "#6e523e", "#8b6c52", "#5f4838", "#92785f"];
      instanceColor.set(part.foliage
        ? foliagePalette[signature % foliagePalette.length]
        : barkPalette[signature % barkPalette.length]);
      mesh.setColorAt(index, instanceColor);
...
function selectWatershedGroundcover(instances: PlacementTuple[], mobile: boolean, tier: WorldQualityTier) {
  ...
  const eligible = instances.filter((placement) => {
    ...
    const shorelineDistance = lakeBoundaryDistance(placement[2], placement[4]);
    if (shorelineDistance <= 0.98 || shorelineDistance >= 1.37) return false;
    const angle = Math.atan2(
      (placement[4] - LAKE_CENTER.z) / LAKE_RADIUS.z,
      (placement[2] - LAKE_CENTER.x) / LAKE_RADIUS.x,
    );
    const habitat = Math.sin(angle * 3.0 + 0.45) * 0.52
      + Math.sin(angle * 7.0 - 1.1) * 0.31
      + Math.sin(placement[2] * 0.047 - placement[4] * 0.021) * 0.17;
    return habitat > -0.18;
  });
...
function selectRegionalHabitatGroundcover(
  ...
  // Balanced keeps the complete already-grounded drainage stratum. These
  // anchors are habitat-gated and reuse resident instanced assets, so the gain
  // is visible ground contact rather than uniform scene-wide density.
  return eligible;
}

function watershedGroundcoverSource(placement: PlacementTuple, index: number): WatershedGroundcoverKey {
  const signature = Math.abs(Math.round(placement[2] * 2.9 + placement[4] * 4.7 + index * 23));
  if (signature % 10 < 4) return "fern";
  if (signature % 10 < 7) return "shrub";
  return "rock";
}
...
function regionalGroundcoverSource(placement: PlacementTuple, index: number, zone: "ridge" | "valley" | "alpine"): WatershedGroundcoverKey {
  const signature = Math.abs(Math.round(placement[2] * 2.7 + placement[4] * 4.3 + index * 19));
  const drainageRatio = zone === "alpine"
...
function DetailedWatershedGroundcover({ placements, shadows }: {
  ...
  ], [fern.scene, rock.scene, shrub.scene]);
  const bySource = useMemo(() => {
  ...
  useEffect(() => {
    const host = window as Window & { __MADAGIN_LITTORAL_SUCCESSION_CE__?: Record<string, unknown> };
    host.__MADAGIN_LITTORAL_SUCCESSION_CE__ = {
      candidate: "CE",
      clusteredHabitat: true,
      fern: bySource.get("fern")?.length ?? 0,
      groundedPlacements: placements.length,
      openShoreReaches: true,
    ...
    document.documentElement.dataset.madaginLittoralSuccessionCe = JSON.stringify(
      host.__MADAGIN_LITTORAL_SUCCESSION_CE__,
    );
    dispatchStage(3, "lake-candidate-ce-clustered-littoral-succession-ready", "lake");
    return () => parts.forEach((part) => {
      (Array.isArray(part.material) ? part.material : [part.material]).forEach((material) => material.dispose());
    });
  }, [bySource, parts, placements.length]);
  return (
    <group name={`Madagin Candidate CE clustered fern, shrub, and basalt littoral succession · ${placements.length}`}>
      {parts.flatMap((part, index) => {
        const sourcePlacements = bySource.get(part.sourceKey) ?? [];
        return sourcePlacements.length ? (
...
function DetailedRegionalHabitatGroundcover({ placements, shadows, zone }: {
  ...
  const bySource = useMemo(() => {
    const result = new Map<WatershedGroundcoverKey, PlacementTuple[]>();
    placements.forEach((placement, index) => {
      const sourceKey = regionalGroundcoverSource(placement, index, zone);
      result.set(sourceKey, [...(result.get(sourceKey) ?? []), placement]);
    });
    return result;
  }, [placements, zone]);
  useEffect(() => {
    ...
    const key = zone === "ridge" ? "ridgeDrainage" : zone === "valley" ? "easternValleyCatchment" : "alpineSuccession";
...
function selectSourceQualityIslandTreePlacements(
  instances: PlacementTuple[],
  mobile: boolean,
  tier: WorldQualityTier,
  zone: V116Zone,
  ...
  if (mobile || tier === "conservative" || zone === "alpine") return [];
  const limit = zone === "valley" ? 34 : zone === "ridge" ? 26 : 14;
  return instances
    .filter((placement) => placement[1] === 0 && placement[0] < 8)
    .filter((placement) => {
      const signature = Math.abs(Math.round(
        placement[2] * 0.29 + placement[4] * 0.17 + placement[9] * 31,
      ));
      return signature % 9 < 3;
    })
    .sort((left, right) => Math.abs(left[2]) - Math.abs(right[2]))
    .slice(0, limit);
}

function InstancedIslandTreeImpostorBatch({ angleOffset, material, placements }: {
  angleOffset: number;
  material: Material;
  placements: PlacementTuple[];
}) {
  ...
  useLayoutEffect(() => {
    ...
    const dummy = new Object3D();
    placements.forEach((placement, index) => {
      const signature = Math.abs(Math.round(placement[2] * 0.37 + placement[4] * 0.21 + index * 17));
      const width = (placement[6] + placement[8]) * (5.25 + (signature % 5) * 0.22);
      const height = placement[7] * (7.1 + ((signature * 3) % 4) * 0.28);
      dummy.position.set(placement[2], placement[3] + height * 0.49, placement[4]);
      dummy.rotation.set(0, placement[5] + angleOffset, 0);
  ...
  return (
    <instancedMesh args={[geometry, material, placements.length]} frustumCulled={false} ref={ref}>
    </instancedMesh>
  );
}

function SourceQualityIslandTreeCanopy({ placements, zone }: {
  placements: PlacementTuple[];
  zone: V116Zone;
}) {
  const textures = useLoader(TextureLoader, SOURCE_QUALITY_ISLAND_TREE_IMPOSTOR_URLS);
  const materials = useMemo(() => textures.map((texture) => {
    ...
    material.name = "Madagin Candidate BT Poly Haven Island Tree 03 impostor";
    return material;
  }), [textures]);
  const batches = useMemo(() => materials.map((material, variant) => ({
    material,
    placements: placements.filter((placement, index) => (
      Math.abs(Math.round(placement[2] * 0.37 + placement[4] * 0.21 + index * 17)) % materials.length
    ) === variant),
  })), [materials, placements]);
  useEffect(() => {
    const host = window as Window & { __MADAGIN_SOURCE_ISLAND_TREE_BT__?: Record<string, unknown> };
    host.__MADAGIN_SOURCE_ISLAND_TREE_BT__ = {
      ...(host.__MADAGIN_SOURCE_ISLAND_TREE_BT__ ?? {}),
      [zone]: {
        crossedPlanesPerPlacement: 2,
        placements: placements.length,
        source: "https://polyhaven.com/a/island_tree_03",
        sourceLicense: "CC0 1.0 Universal",
        sourceType: "scan-derived orthographic impostor",
      },
    };
    document.documentElement.dataset.madaginSourceIslandTreeBt = JSON.stringify(
      host.__MADAGIN_SOURCE_ISLAND_TREE_BT__,
    );
    return () => materials.forEach((material) => material.dispose());
  }, [materials, placements.length, zone]);
  return (
    <group name={`Madagin Candidate BT Island Tree 03 canopy · ${placements.length}`}>
      {batches.flatMap(({ material, placements: batch }, variant) => batch.length ? [
        <InstancedIslandTreeImpostorBatch angleOffset={0} key={`${variant}-a`} material={material} placements={batch} />,
        <InstancedIslandTreeImpostorBatch angleOffset={Math.PI / 2} key={`${variant}-b`} material={material} placements={batch} />,
      ] : [])}
    </group>
  );
}
...
function EcologyChunk({ diagnosticMode, mobile, shadows, tier, zone }: {
  ...
  const sourceQualityIslandTreePlacements = useMemo(
    () => selectSourceQualityIslandTreePlacements(manifest.instances, mobile, tier, zone)
      .filter((placement) => !sourceQualityPachiraSet.has(placement)),
    [manifest.instances, mobile, sourceQualityPachiraSet, tier, zone],
  );
  const sourceQualityIslandTreeSet = useMemo(
  ...
  const detailedVegetationSet = useMemo(
    ...
    [detailedPlacements, sourceQualityIslandTreePlacements, sourceQualityPachiraPlacements],
  );
  ...
  const regionalHabitatGroundcover = useMemo(() => {
    if (zone !== "ridge" && zone !== "valley" && zone !== "alpine") return [];
  ...
  const visible = useMemo(() => manifest.instances.filter((placement, index) => {
    ...
    // Candidate CF opens the normal desktop composition by thinning only the
    // repeated legacy core stratum. The source-quality canopy, grounded
    // riparian succession, and drainage habitat above are retained in full.
    if (tier === "balanced" && !mobile) {
      if (placement[1] === 0) return index % 4 !== 1;
      if (placement[1] === 1) return index % 3 !== 1;
      return index % 2 === 0;
    }
  ...
  return (
    <group name={`Madagin v1.16 ${zone} spatial ecology · ${visible.length} visible instances`}>
      ...
      {sourceQualityGeologyPlacements.length ? (
        <Suspense fallback={null}>
          <SourceQualityGeologyAnchors
            placements={sourceQualityGeologyPlacements}
            shadows={shadows}
            zone={zone}
          />
        </Suspense>
      ...
      {regionalHabitatGroundcover.length && (zone === "ridge" || zone === "valley" || zone === "alpine") ? (
        <Suspense fallback={null}>
...
function createWaterMaterial(kind: "watershed" | "river" | "headwater" | "pool" = "watershed") {
  ...
  const material = new ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        vec3 rippleNormal = normalize(vec3(
          ...
          1.0,
          cos(phaseA) * ${lake ? "0.052" : "0.07"} + cos(phaseB) * ${lake ? "0.078" : "0.125"} + sin(capillary) * ${lake ? "0.016" : "0.0"}
        ));
        float stochasticWave = waterNoise(vWorld.xz * 0.047 + vec2(uTime * 0.011, -uTime * 0.007));
        float stochasticWaveX = waterNoise((vWorld.xz + vec2(0.42, 0.0)) * 0.047 + vec2(uTime * 0.011, -uTime * 0.007));
        float stochasticWaveZ = waterNoise((vWorld.xz + vec2(0.0, 0.42)) * 0.047 + vec2(uTime * 0.011, -uTime * 0.007));
        vec3 stochasticRippleNormal = normalize(vec3(
          (stochasticWave - stochasticWaveX) * 0.72,
          1.0,
    ...
    : river
      ? "mix(mix(vec3(0.12, 0.205, 0.21), vec3(0.09, 0.18, 0.24), riverMouth), atmosphericReflection, 0.34)"
        ...
        float reflectedCanopyBreakup = 0.58 + sin(vWorld.x * 0.061 + vWorld.z * 0.047) * 0.16
          + sin(vWorld.x * -0.029 + vWorld.z * 0.083) * 0.11;
        skyReflection = mix(
          skyReflection,
        ...
        ${lake ? "opacity = mix(0.7, 0.94, basinDepth) * mix(0.88, 1.0, fresnel);" : ""}
  ...
  material.name = headwater
    ...
    : river
      ...
      : lake
        ? "Madagin Candidate CF translucent depth-graded volcanic tarn with anisotropic atmospheric reflection"
  ...
  const material = new ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float light = 0.49 + max(dot(normalize(vNormal), normalize(vec3(-0.42, 0.82, 0.38))), 0.0) * 0.41;
...
function WaterNetwork({ mobile, reducedMotion, shadows, tier, zone }: { mobile: boolean; reducedMotion: boolean; shadows: boolean; tier: WorldQualityTier; zone: JourneyCheckpointId }) {
  ...
  const littoralGeologyPlacements = useMemo(
  ...
  useEffect(() => {
    ...
    const host = window as Window & {
      ...
      __MADAGIN_WATER_REALISM_CE__?: Record<string, unknown>;
      __MADAGIN_WATER_REALISM_CF__?: Record<string, unknown>;
    ...
    host.__MADAGIN_WATER_REALISM_BY__ = {
      candidate: "BY",
      lakeBed: {
    ...
    host.__MADAGIN_WATER_REALISM_CE__ = {
      candidate: "CE",
      continuousHydrology: true,
      lakeShore: {
        clusteredSourceGeology: true,
        openShoreReaches: true,
        sourceGeologyPlacements: littoralGeologyPlacements.length,
        sourceGeologyScaleReduction: 0.61,
        successionAuthority: "shared analytic lake boundary with habitat-gated fern, shrub, and basalt placement",
      },
      protectedWaterCandidate: "CC",
      protectedOceanCandidate: "CD",
    };
    host.__MADAGIN_WATER_REALISM_CF__ = {
      candidate: "CF",
      continuousHydrology: true,
      lake: {
        depthWrite: true,
        interiorOpacity: 0.94,
        littoralOpacity: 0.7,
        retainedAnisotropicReflection: true,
        substrateVisibleAtShore: true,
      },
      protectedOceanCandidate: "CD",
      waterfall: {
        braidedTransparentGaps: true,
        lowerContinuousWhiteAuthority: true,
        minimumFragmentAlpha: 0.07,
        retainedConnectedGeometry: true,
      },
    };
    ...
    document.documentElement.dataset.madaginWaterRealismCe = JSON.stringify(host.__MADAGIN_WATER_REALISM_CE__);
    document.documentElement.dataset.madaginWaterRealismCf = JSON.stringify(host.__MADAGIN_WATER_REALISM_CF__);
    ...
    return () => {
      ...
      delete host.__MADAGIN_WATER_REALISM_CE__;
      delete host.__MADAGIN_WATER_REALISM_CF__;
  ...
  return (
    <group name="Madagin Candidate CF translucent tarn and broken-runnel waterfall over Candidate CE clustered littoral succession">
      {littoralGeologyPlacements.length ? (
        <Suspense fallback={null}>
          <SourceQualityGeologyAnchors
            placements={littoralGeologyPlacements}
            shadows={shadows}
            zone="lake"
          />
        </Suspense>
  ...
  return new ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        float alpha=edge*smoothstep(0.27,0.67,body+edge*0.26)*mix(0.76,1.0,baseDensity)*uOpacity*1.12;
...
function createTerrainMistMaterial(opacity: number, seed: number) {
  return new ShaderMaterial({
    depthTest: true,
    depthWrite: false,
    // Rendering both sides of a transparent volume double-blends its back
    // faces and exposes long triangle chords as false weather bands. The
    // camera remains outside these broad volumes, so one outward-facing shell
    // provides the intended terrain-contact veil without self-sorting seams.
    side: FrontSide,
    toneMapped: true,
    transparent: true,
    uniforms: { uOpacity: { value: opacity }, uSeed: { value: seed }, uTime: { value: 0 } },
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        float alpha=envelope*smoothstep(0.19,0.57,macro*0.68+breakup*0.32)*mix(0.66,1.0,floorFade)*uOpacity*1.16;
        if(alpha<0.003)discard;
        vec3 mistColor=mix(vec3(0.36,0.46,0.47),vec3(0.72,0.77,0.74),clamp(macro*0.72+vLocal.y*0.18,0.0,1.0));
        gl_FragColor=vec4(mistColor,alpha);
        #include <tonemapping_fragment>
        #include <colorspace_fragment>
      }
...
const CLOUD_BANK_LOBES = [
  { position: [-0.24, -0.02, 0.04] as [number, number, number], scale: [0.72, 0.82, 0.68] as [number, number, number] },
  { position: [0.04, 0.08, -0.12] as [number, number, number], scale: [0.86, 1.02, 0.78] as [number, number, number] },
  { position: [0.27, -0.06, 0.1] as [number, number, number], scale: [0.66, 0.74, 0.7] as [number, number, number] },
  { position: [0.1, -0.15, 0.22] as [number, number, number], scale: [0.52, 0.58, 0.56] as [number, number, number] },
] as const;

const TERRAIN_CONTACT_MIST_BANKS = [
  { authority: "ridge-drainage", position: [-72, 48, -205] as [number, number, number], scale: [172, 36, 62] as [number, number, number], opacity: 0.2 },
  { authority: "valley-catchment", position: [-88, 16, -720] as [number, number, number], scale: [278, 40, 96] as [number, number, number], opacity: 0.32 },
  { authority: "waterfall-plunge", position: [146, -6, -700] as [number, number, number], scale: [118, 34, 64] as [number, number, number], opacity: 0.31 },
  ...
  { authority: "western-collapse-hollow", position: [-342, 74, -812] as [number, number, number], scale: [138, 46, 92] as [number, number, number], opacity: 0.15 },
  { authority: "eastern-collapse-hollow", position: [386, 82, -672] as [number, number, number], scale: [146, 48, 88] as [number, number, number], opacity: 0.14 },
  { authority: "summit-crown-saddle", position: [608, 212, -1418] as [number, number, number], scale: [172, 54, 92] as [number, number, number], opacity: 0.13 },
...
function V116Atmosphere({ reducedMotion, shadows, tier }: { reducedMotion: boolean; shadows: boolean; tier: WorldQualityTier }) {
  ...
  const materials = useMemo(
    () => clouds.map((cloud, index) => createCloudMaterial(cloud.opacity * 0.58, index + 1)),
    [clouds],
  );
  ...
  useEffect(() => {
    const host = window as Window & {
      ...
      __MADAGIN_OROGRAPHIC_WEATHER_CE__?: Record<string, unknown>;
    ...
    host.__MADAGIN_OROGRAPHIC_WEATHER_BX__ = {
      candidate: "BX",
      authorities: mistBanks.map((mist) => mist.authority),
      cloudBanks: clouds.length,
      groundedBanks: mistBanks.length,
      method: "terrain-coupled anisotropic mist volumes under a single physical sky and lighting authority",
    };
    document.documentElement.dataset.madaginOrographicWeatherBx = JSON.stringify(host.__MADAGIN_OROGRAPHIC_WEATHER_BX__);
    host.__MADAGIN_OROGRAPHIC_WEATHER_BZ__ = {
      candidate: "BZ",
      authorities: mistBanks.map((mist) => mist.authority),
      cloudBanks: clouds.length,
    ...
    host.__MADAGIN_OROGRAPHIC_WEATHER_CE__ = {
      candidate: "CE",
      authorities: mistBanks.map((mist) => mist.authority),
      cloudBanks: clouds.length,
      cloudLobes: clouds.length * CLOUD_BANK_LOBES.length,
      groundedBanks: mistBanks.length,
      lightingAuthority: "reduced broad hemisphere fill with stronger warm directional relief and retained cool sky bounce",
      method: "each stretched cloud shell is replaced by four offset low-opacity erosion volumes while retained mist remains coupled to named terrain authorities",
  ...
  }, [clouds, materials, mistBanks, mistMaterials, tier]);
  return (
    <group name="Madagin v1.16 single physical atmosphere and lighting authority">
      ...
      <directionalLight
        ...
        shadow-mapSize-height={tier === "high" ? 2048 : 1536}
        shadow-mapSize-width={tier === "high" ? 2048 : 1536}
      />
      <directionalLight color="#bad2d5" intensity={0.18} position={[190, 150, -220]} />
      <group ref={cloudGroup}>
        {clouds.map((cloud, index) => (
          <group key={cloud.position.join(":")} position={cloud.position} scale={cloud.scale}>
            {CLOUD_BANK_LOBES.map((lobe, lobeIndex) => (
              <mesh
                key={lobeIndex}
                material={materials[index]}
                position={lobe.position}
                rotation={[0, (index * 0.41 + lobeIndex * 0.67) % Math.PI, 0]}
                scale={lobe.scale}
              >
      ...
      <group name={`Madagin Candidate CE basin-to-ridge coupled weather · ${mistBanks.length} grounded banks`} ref={mistGroup}>
...
function Ocean({ mobile, reducedMotion, tier }: { mobile: boolean; reducedMotion: boolean; tier: WorldQualityTier }) {
  ...
  useEffect(() => {
    ...
    const evidence = {
      candidate: "BZ",
      breakerBands: 4,
      coastlineAuthority: "shared four-octave analytic western coastline consumed by the connected coastal remesh and ocean shader",
      coastalVoidClosed: true,
      displacedBreakerBands: 3,
      foamTongues: true,
      nearshoreBackwash: true,
      sedimentBearingShallows: true,
      surfaceSegments: segments,
      surfPersistence: "four spatially broken crest and backwash authorities",
    };
    ...
    return () => {
      ...
      if (host.__MADAGIN_OCEAN_REALISM_BZ__ === evidence) {
...
export function RidgeProductionV116({ diagnosticMode, mobile, reducedMotion, shadows, showOcean, tier, zone }: RidgeProductionV116Props) {
  ...
  useEffect(() => {
    ...
    const host = window as Window & {
      ...
      __MADAGIN_REALISM_CE__?: Record<string, unknown>;
      __MADAGIN_REALISM_CF__?: Record<string, unknown>;
    ...
    host.__MADAGIN_REALISM_BY__ = {
      candidate: "BY",
      categories: {
        atmosphereAndLighting: "twelve terrain-contact weather authorities, deeper sky separation, lower ambient fill, and a stronger warm key under one physical lighting authority",
        ecologyAndGrounding: "expanded replacement of legacy canopy with the 109,999-triangle licensed three-dimensional Island Tree 01 family, including foliage-cast contact shadows, alongside retained Pachira and Island Tree 03 anchors",
        materialScale: "world-projected PBR ground with cross-bedded scarps, joint networks, colluvial fans, and area-weighted source normals",
        terrainStructure: "two adaptive Valley escarpment refinement passes plus physical collapse hollows, retaining ribs, branching gullies, and a twice-refined less-relaxed Alpine crown on connected source surfaces",
        waterIntegration: "cellular-broken tarn reflection, denser littoral geology, a closer normal-journey composition for the continuous two-stage widened waterfall and three impact arcs, and a three-breaker ocean with foam tongues",
      },
    ...
    host.__MADAGIN_REALISM_BZ__ = {
      candidate: "BZ",
      categories: {
        atmosphereAndLighting: "fourteen terrain-contact authorities including two coast-contact weather banks under the retained physical sky",
        coastalEcology: "north and south coastal succession with bounded licensed three-dimensional Island Tree 01 age, width, and wind-pruned silhouettes",
        coastalStructure: "one connected terrain-to-shore remesh spans the Ridge and southern Valley instead of exposing the former sky-colored coastal wedge",
        materialScale: "retained world-projected volcanic terrain material continues across the new single-indexed coastal surface",
        waterIntegration: "terrain and ocean share one coastline; sediment-bearing shallows, four spatially broken breaker bands, foam tongues, and backwash meet the remeshed shore",
      },
      coastlineAuthority: "shared four-octave analytic western coastline",
      detachedTerrainShells: false,
      exposedCoastalVoidClosed: true,
    ...
    host.__MADAGIN_REALISM_CE__ = {
      candidate: "CE",
      categories: {
        atmosphereAndLighting: "five retained cloud banks become twenty offset low-opacity erosion lobes; reduced broad fill and stronger directional relief improve terrain depth without increasing concealing mist",
        ecologyAndGrounding: "the former rock-only nearshore stratum becomes habitat-gated fern, shrub, and basalt succession with open shore reaches; source-quality geology is smaller, lower, and clustered",
        materialScale: "continuous world-space basalt, colluvium, drainage, and talus fields replace most remaining source-atlas chroma authority and add a second mesoscopic normal response",
        terrainStructure: "bounded dual-frequency weathering and narrow subtractive rills physically break Ridge, Valley, and Alpine source surfaces while decoded seams and all water corridors remain protected",
        waterIntegration: "the accepted CC hydrology and CD ocean are retained while fourteen high-tier or eight balanced source-geology anchors replace the former near-continuous thirty-two/high or twenty-four/balanced lake-edge ring",
      },
      cloudLobesPerBank: CLOUD_BANK_LOBES.length,
      coastlineAuthority: "retained Candidate CD shared four-octave analytic western coastline",
      detachedTerrainShells: false,
      exposedCoastalVoidClosed: true,
      lakeRadiusMeters: [LAKE_RADIUS.x, LAKE_RADIUS.z],
      protectedOceanCandidate: "CD",
    ...
    host.__MADAGIN_REALISM_CF__ = {
      candidate: "CF",
      categories: {
        atmosphereAndLighting: "retained Candidate CE twenty-lobe cloud breakup and directional terrain-depth lighting",
        ecologyAndGrounding: "normal desktop presentation thins only repeated legacy core canopy while retaining source-quality trees and full drainage, riparian, and clustered littoral succession",
        materialScale: "retained Candidate CE continuous basalt-to-colluvium weathering and mesoscopic normal response",
        terrainStructure: "retained Candidate CE physical Ridge, Valley, and Alpine weathering with decoded seams and water corridors protected",
        waterIntegration: "the lake now exposes its authored depth-graded substrate through a 0.70-to-0.94 optical column, while the connected waterfall lowers continuous white authority and opens stronger transparent runnel gaps",
      },
      coastlineAuthority: "retained Candidate CD shared four-octave analytic western coastline",
      detachedTerrainShells: false,
      exposedCoastalVoidClosed: true,
    ...
    document.documentElement.dataset.madaginRealismCe = JSON.stringify(host.__MADAGIN_REALISM_CE__);
    document.documentElement.dataset.madaginRealismCf = JSON.stringify(host.__MADAGIN_REALISM_CF__);
    document.documentElement.dataset.madaginLivingWindV116 = "spatial-phased-vertex-wind";
  }, [chunks, terrainChunks, zone]);
  return (
    <group name={`Madagin Ridge-to-Valley v1.16 + Candidate CF cumulative basin-to-water realism world · ${zone} · ${chunks.join("+")} · ${showOcean ? "ocean focus" : "journey focus"}`}>
