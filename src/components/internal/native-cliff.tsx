...
export const NATIVE_CLIFF = {version:field.version, sourceGridMeters:field.sourceGridMeters, width:field.width, depth:field.depth, collarMeters:field.collarMeters, verticalTranslation:field.verticalTranslation};
const smooth=(x:number)=>{const t=Math.max(0,Math.min(1,x));return t*t*(3-2*t);};

export function nativeCliffWeight(x:number,z:number) {
...
export function nativeCliffElevation(x:number,z:number) {
...
export function applyNativeCliff(geometry:BufferGeometry) {
  if(geometry.userData.nativeCliff)return geometry;
  const p=geometry.getAttribute("position");let changed=0,maxDelta=0;
  for(let i=0;i<p.count;i++){
    const x=p.getX(i),z=p.getZ(i),weight=nativeCliffWeight(x,z);if(weight===0)continue;
    const y=p.getY(i),next=y+(nativeCliffElevation(x,z)-y)*weight;
  ...
  geometry.userData.nativeCliff={...NATIVE_CLIFF,changedVertices:changed,maxDelta,sourceInteriorPreserved:true,authoredBoundaryCollar:true};
  return geometry;
}
...
export function nativeCliffSampler(geometry:BufferGeometry) {
...
export function createNativeCliffPlants(geometry:BufferGeometry,compact:boolean) {
  const sample=nativeCliffSampler(geometry),plants:number[][]=[];
  let seed=908071;const random=()=>{seed=(Math.imul(seed,1664525)+1013904223)>>>0;return seed/4294967296;};
  ...
  for(let z=field.z0+6;z<field.z0+field.depth-6;z+=step)for(let x=field.x0+6;x<field.x0+field.width-6;x+=step){
    ...
    plants.push([0,layer,px,y-.06,pz,random()*Math.PI*2,1,scale,1,random(),0,0]);
  }
  return plants;
}

export function NativeCliffPlants({geometry,compact=false,shadows}:{geometry:BufferGeometry|null;compact?:boolean;shadows:boolean}) {
  const plants=useMemo(()=>geometry?createNativeCliffPlants(geometry,compact):[],[compact,geometry]);
  useEffect(()=>{
    if(!geometry)return;
    document.documentElement.dataset.madaginNativeCliff=JSON.stringify({...geometry.userData.nativeCliff,...NATIVE_CLIFF,compact,plants:plants.length,triangleGrounded:true});
    return ()=>{delete document.documentElement.dataset.madaginNativeCliff;};
  ...
  return plants.length?<Suspense fallback={null}><RootedTrees placements={plants} zone="native-cliff" shadows={shadows} compact={compact}/></Suspense>:null;
}
