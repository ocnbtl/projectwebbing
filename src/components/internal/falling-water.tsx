...
export const FALLING_WATER = {version:"cascade-volume-2",gravity:9.81,entrySpeed:2.4} as const;
...
export function createFallingWaterGeometry(top:Point,bottom:Point,lipHalfWidth:number) {
  ...
  for(let row=0;row<=rows;row++){
    ...
    const volume=Math.max(0,Math.min(1,p/.065,(1-p)/.085));
    for(let col=0;col<=columns;col++){
      ...
      // Endpoints stay fixed; the body remains close to the retained chute.
      ...
      // The retained lip/pool rings blend into cross-sections normal to flow.
      // Unequal shoulders have real depth, not a thin sheet laid over the cliff.
      const radius=(.38+width*.27)*(1+Math.exp(-(((p-.86)/.13)**2))*.6);
      const lobes=1+Math.sin(angle*3+p*16)*.19+Math.sin(angle*5-p*23)*.09;
      const v=new Vector3(section.x,centerY,z)
        .addScaledVector(side,a*width*edge)
        .addScaledVector(outward,(.68+Math.sin(angle)*lobes)*radius);
      const original=new Vector3(x,cascadeLevel(x,z)+Math.sin(angle)*thickness+lobe,z);
      // The cross-slope guide can dip sharply at a shoulder. Bound its lift
      // when rotating into the flow plane so the body cannot bridge that dip.
      const displacement=v.sub(original).clampLength(0,4.5).multiplyScalar(volume);
      original.add(displacement);positions.push(original.x,original.y,original.z);
  ...
  geometry.userData.waterfallBody={version:FALLING_WATER.version,guideVersion:CASCADE_VERSION,top,bottom,lipHalfWidthMeters:lipHalfWidth,rows,columns,travelSeconds:fallingTravelTime(top.y-bottom.y),gravity:FALLING_WATER.gravity,entrySpeed:FALLING_WATER.entrySpeed,contactPathAuthored:true,connectedGeometry:true,centralSpineContinuous:true,terrainGuide:true,topology:"Unequal volumetric cross sections perpendicular to the retained flow guide; exact lip/pool rings. No fluid simulation."};
  return geometry;
}
const NOISE=`
...
export function createFallingWaterMaterial(sun:Vector3) {
  return new ShaderMaterial({name:"Madagin gravity-timed falling water",side:DoubleSide,transparent:true,depthWrite:false,uniforms:{uTime:{value:0},uSun:{value:sun.clone()}},
    ...
    void main(){
      ...
      float broad=noise(vec2(vUv.x*6.7,clock*8.1));
      float folded=noise(vec2(vUv.x*13.3+broad*1.8,clock*21.7));
      ...
      float alpha=edge*mix(.36+body*.24,.48+body*.31,air)*mix(1.,breakup,.34*air);
      vec3 normal=normalize(cross(dFdx(vWorld),dFdy(vWorld)));
      float light=.64+.36*abs(dot(normal,uSun));
      vec3 color=mix(vec3(.17,.27,.27),vec3(.87,.92,.89),clamp(.13+air*.6+body*.14+bubble*.13,0.,1.))*light;
...
export function createFallingImpactMaterial() {
  return new ShaderMaterial({name:"Madagin advecting impact foam",side:DoubleSide,transparent:true,depthWrite:false,uniforms:{uTime:{value:0}},
    ...
    void main(){
      ...
      float core=exp(-r*r*8.)*(.68+patches*.32);
      ...
      float foam=clamp(core*.94+wake*.68,0.,1.);
