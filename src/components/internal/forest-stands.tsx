...
export const FOREST_STANDS_VERSION="forest-stands-1";
...
export function createForestStandPlacements(geometry:BufferGeometry,compact:boolean,zone:"ridge"|"valley") {
  ...
  for(let i=0;i<(index?.count??p.count);i+=3){
    ...
    const x0=Math.max(-270,Math.min(ax,bx,cx)),x1=Math.min(630,Math.max(ax,bx,cx));
    const z0=Math.max(zone==="ridge"?-308:-1020,Math.min(az,bz,cz)),z1=Math.min(zone==="ridge"?275:-316,Math.max(az,bz,cz));
    ...
    for(let iz=Math.floor(z0/step);iz<=Math.floor(z1/step);iz++)for(let ix=Math.floor(x0/step);ix<=Math.floor(x1/step);ix++){
      ...
      if(support<.64||y>190||forestWetExclusion(x,y,z))continue;
      ...
      const density=grove*smooth((support-.64)/.22)*(1-smooth((y-100)/110));
      const rank=hash(ix,iz,3);if(rank>density||(compact&&hash(ix,iz,7)>.66))continue;
      const young=hash(ix,iz,4)<.26,scale=(young?.55:.8)+hash(ix,iz,5)*.38;
      cells.set(key,{p:[0,young?1:0,x,y-.12,z,hash(ix,iz,6)*Math.PI*2,1,scale,1,rank,0,0],triangle:[ax,ay,az,bx,by,bz,cx,cy,cz]});
    }
  }
  const witnesses=[...cells.values()];
  return {placements:witnesses.map(v=>v.p),witnesses};
}

export function ForestStands({geometry,compact=false,shadows,zone}:{geometry:BufferGeometry|null;compact?:boolean;shadows:boolean;zone:"ridge"|"valley"}) {
