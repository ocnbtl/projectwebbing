...
export const FOREST_STANDS_VERSION="forest-stands-3";
...
export function createForestStandPlacements(geometry:BufferGeometry,compact:boolean,zone:ForestZone) {
  ...
  for(let i=0;i<(index?.count??p.count);i+=3){
    ...
    const x0=Math.max(zone==="alpine"?-235:-270,Math.min(ax,bx,cx)),x1=Math.min(zone==="alpine"?900:zone==="valley"?785:630,Math.max(ax,bx,cx));
    ...
    // A second, bounded set of smaller stand cells resolves the narrow dry
    // banks. They join the same instanced forest rather than adding a ring mesh
    // or a population which appears as the camera approaches.
    const bankTriangle=zone==="valley"&&x0<190&&x1>-200&&z0<-755&&z1>-1020;
    for(const spacing of bankTriangle?[step,7.5]:[step])
    for(let iz=Math.floor(z0/spacing);iz<=Math.floor(z1/spacing);iz++)for(let ix=Math.floor(x0/spacing);ix<=Math.floor(x1/spacing);ix++){
      const bank=spacing!==step;
      const x=(ix+.12+hash(ix,iz,1)*.76)*spacing,z=(iz+.12+hash(ix,iz,2)*.76)*spacing;
      const u=((bz-cz)*(x-cx)+(cx-bx)*(z-cz))/determinant,v=((cz-az)*(x-cx)+(ax-cx)*(z-cz))/determinant;
      if(u<0||v<0||u+v>1)continue;
      const y=u*ay+v*by+(1-u-v)*cy,key=`${bank?'bank:':''}${ix},${iz}`;
      if((top.get(key)??-Infinity)>=y)continue;
      top.set(key,y);cells.delete(key);
      const lakeDistance=lakeBoundaryDistance(x,z);
      if(bank&&(lakeDistance<1.065||lakeDistance>1.72||y>-8))continue;
      ...
      const edge=zone==="alpine"?Math.max(
        smooth((x-300)/45)*smooth((900-x)/55)*smooth((z+1610)/55)*smooth((-1008-z)/45),
        smooth((x+235)/45)*smooth((300-x)/55)*smooth((z+1460)/55)*smooth((-1008-z)/24)
      ):1;
      const density=(bank?Math.min(.92,grove+.2):grove)*smooth((support-.64)/.22)*(upland?(1-exposure)*edge:(1-smooth((y-100)/110)));
      const rank=hash(ix,iz,3);if(rank>density||(compact&&hash(ix,iz,7)>.66))continue;
      const young=hash(ix,iz,4)<(bank?.42:upland?.44:.26),scale=((young?.55:.8)+hash(ix,iz,5)*.38)*(bank?.95:upland?.85-exposure*.3:1);
      cells.set(key,{p:[0,young?1:0,x,y-.12,z,hash(ix,iz,6)*Math.PI*2,1,scale,1,rank,0,0],triangle:[ax,ay,az,bx,by,bz,cx,cy,cz]});
    }
  }
  const witnesses=[...cells.entries()].filter(([key,{p}])=>{
    if(key.startsWith('bank:')){
      // Reject a supplemental root near an established stand; spacing is
      // evaluated in world metres even though the two lattices differ.
      const ix=Math.floor(p[2]/step),iz=Math.floor(p[4]/step);
      for(let dz=-1;dz<=1;dz++)for(let dx=-1;dx<=1;dx++){
        const existing=cells.get(`${ix+dx},${iz+dz}`)?.p;
        if(existing&&Math.hypot(p[2]-existing[2],p[4]-existing[4])<5.5)return false;
      }
      return true;
    }
