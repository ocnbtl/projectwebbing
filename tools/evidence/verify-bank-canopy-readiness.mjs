...
const origin=process.env.MADAGIN_REVIEW_ORIGIN??'http://127.0.0.1:3164';
const out=path.resolve('output/playwright/madagin-world-progress',process.env.MADAGIN_EVIDENCE_CYCLE??'bank-canopy-release-20260909',process.env.MADAGIN_PUBLIC_EVIDENCE_LABEL??'canopy-ordering-local');
...
try {
 for(const {width,constrained} of [{width:1440,constrained:false},{width:390,constrained:false},{width:1440,constrained:true}])for(const held of constrained?['tree-model']:['tree-model','tree-textures']){
  ...
  const asset=held==='tree-model'?'**/world/rooted-trees-v1/*-far.glb':'**/world/rooted-trees-v1/textures/*.webp';
