...
const origin=process.env.MADAGIN_REVIEW_ORIGIN??'http://127.0.0.1:3183';
const phase=process.env.MADAGIN_TERRAIN_PHASE??'local';
const out=path.resolve(process.env.MADAGIN_TERRAIN_EVIDENCE_ROOT??'output/releases/madagin-ground-substrate-20260910');
...
try {
  for(const width of [1440,390]) {
        ...
        p[method]=function(...args) {
          ...
          if(current) {
            ...
            if(!current.textures) {
              ...
              current.textures=[];
              try {
                for(const name of ['uCliffColor','uCliffNormal','uCliffResponse','uGroundColor','uGroundNormal','uGroundResponse']) {
    ...
    assert.equal(observed.fallingWater.version,'cascade-streams-1');assert.equal(observed.fallingWater.guideVersion,'contact-cascade-1');assert.match(observed.water.fragment,/fwidth\(bubbles\)/);assert.doesNotMatch(observed.water.fragment,/float channels=/);assert.equal(observed.fallingWater.terrainGuide,true);assert.ok(observed.water.linked&&observed.water.draws>0&&observed.water.aerationBound&&observed.alpine);assert.match(observed.fragment,/float cascadeWet/);const initialTime=observed.water.time;await page.waitForTimeout(700);const laterTime=await page.evaluate(()=>window.__MADAGIN_CASCADE_CHECK__.time);assert.ok(laterTime>initialTime);observed.water.laterTime=laterTime;assert.equal(observed.nativeValley.version,'native-valley-2');
    assert.equal(observed.nativeValley.compact,width===390);
    assert.ok(observed.nativeValley.changedVertices>8000&&observed.nativeValley.plants===(width===390?58:131)&&observed.nativeValley.triangleGrounded&&observed.nativeValley.sourceInteriorPreserved);
    ...
    if(width===390){
      ...
      await page.waitForFunction(()=>JSON.parse(document.documentElement.dataset.madaginPublicCamera).progress===.98,null,{timeout:30000});
      const summit=await page.evaluate(()=>JSON.parse(document.documentElement.dataset.madaginCompactJourneySeamV116).alpineBoundary);
      assert.deepEqual(summit,journey,'Retain the same connected terrain through Summit');
    ...
    assert.match(observed.fragment,/groundSlope\(uCliffNormal, vGroundWorld\.zy, 80\.0\)/);assert.equal(observed.surface.groundTileMeters,15);assert.match(observed.fragment,/groundSlope\(uGroundNormal, vGroundWorld\.zy, 15\.0\)/);assert.match(observed.fragment,/mix\(response\.g, groundResponse\.g, cover\)/);
    assert.match(observed.fragment,/mat3\(viewMatrix\) \* detailGradient/);
    assert.match(observed.fragment,/supportSlope \+ \(\.7 - response\.r\) \* \.12/);
    assert.ok(observed.textures.every(t=>t.bound&&t.size?.width===(t.name==='uGroundNormal'||t.name==='uGroundResponse'||(width===390&&t.name==='uGroundColor')?256:observed.surface.textureResolution)&&t.min===9987&&t.mag===9729&&t.wrapS===10497&&t.wrapT===10497));
    assert.equal(new Set(observed.textures.map(t=>t.unit)).size,6);
