...
const out=path.resolve('output/playwright/madagin-world-progress',process.env.MADAGIN_EVIDENCE_CYCLE??'channel-rocks-release-20260908',process.env.MADAGIN_PUBLIC_EVIDENCE_LABEL??'rock-ordering-local');
await fs.mkdir(out,{recursive:true});
const report={origin,at:new Date().toISOString(),cases:[]};
const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true});
try {
 for(const {width,constrained} of [{width:1440,constrained:false},{width:390,constrained:false},{width:1440,constrained:true}])for(const held of constrained?['rock-model']:['rock-model','rock-textures']){
  ...
  await context.addInitScript(()=>{window.__MADAGIN_RIDGE_STAGES_V116__=[{at:-1,zone:'ridge',stage:2,label:'channel-rocks-ready'},{at:-1,zone:'ridge',stage:0,label:'ridge-terrain-ready'},{at:-1,zone:'ridge',stage:2,label:'ridge-ecology-ready'}];});
  ...
  const asset=held==='rock-model'?'**/world/canopy-v1/moss-rock.glb':'**/world/canopy-v1/textures/dd2503789089fb72ef469efd14364d393e769c0de8781a251475cac7e0b6f51d.jpeg';
  ...
  try{
   ...
   assert.ok(await page.evaluate(()=>JSON.parse(document.documentElement.dataset.madaginChannelRocks).version==='channel-rocks-1'));
   assert.equal(await page.evaluate(()=>JSON.parse(document.documentElement.dataset.madaginChannelRocks).compact),width===390||constrained);
   if(constrained)assert.equal(await page.locator('[data-public-world]').getAttribute('data-quality-tier'),'conservative');
   assert.ok(await page.evaluate(()=>window.__MADAGIN_RIDGE_STAGES_V116__.some(s=>s.at>0&&s.label==='channel-rocks-ready')));
