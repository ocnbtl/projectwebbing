...
const root=process.env.MADAGIN_CROWN_EVIDENCE??'output/releases/madagin-canopy-mass-20260913';
...
if(!await fs.stat(baselinePath).catch(()=>null)){
 ...
 await fs.writeFile(baselinePath,execFileSync('git',['show','d56e8d0e8bf8b371b2568cd3481da1a070ae43bd:src/components/internal/branch-crowns.ts']));
}
...
for(const variant of [0,1,2,3])for(const stand of [false,true]){
 ...
 assert.ok(a.leafArea>b.leafArea*1.5&&a.leafArea<b.leafArea*3.5,'Bound the connected foliage area change');
 assert.equal(a.rootY,b.rootY);assert.ok(a.triangles<32000&&a.triangles<b.triangles*1.55,'Bound per-model geometry cost');
 assert.ok(a.size.every((s,i)=>s/b.size[i]>.75&&s/b.size[i]<1.25),'No hidden whole-tree enlargement');
 ...
 for(let i=1;i<lp.count;i+=7){const p=new Vector3().fromBufferAttribute(lp,i);let nearest=Infinity;for(let j=1;j<centers.length;j++){const start=centers[j-1],d=centers[j].clone().sub(start),t=Math.max(0,Math.min(1,p.clone().sub(start).dot(d)/Math.max(1e-12,d.lengthSq())));nearest=Math.min(nearest,p.distanceTo(start.clone().addScaledVector(d,t)));}maximumAttachmentError=Math.max(maximumAttachmentError,nearest);}
 ...
 rows.push({variant,stand,before:b,after:a,maximumAttachmentError,shelter});
}
