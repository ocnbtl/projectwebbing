...
try {
 for(const viewport of [{width:1440,height:900},{width:390,height:844}]) {
  ...
  const metrics=await page.evaluate(()=>({
   bankUnderstory:document.documentElement.dataset.madaginBankUnderstory?JSON.parse(document.documentElement.dataset.madaginBankUnderstory):null,
  ...
  assert.equal(metrics.bankUnderstory?.version,'bank-understory-1');
  assert.equal(metrics.bankUnderstory.ferns,viewport.width===1440?820:440);
  assert.equal(metrics.rootedTrees['bank-understory'].count,viewport.width===1440?160:80);
