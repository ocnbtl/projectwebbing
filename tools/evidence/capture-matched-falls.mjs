...
const root=path.resolve('output/playwright/madagin-world-progress/falls-release-20260907');
await fs.mkdir(root,{recursive:true});
const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true});
const report={at:new Date().toISOString(),method:'Optimized public renderer. Pause UI, set existing React MotionValue to fixed public-rail progress. Sun and shadow settings unchanged; waterfall geometry and motion intentionally change. Animation phases not synchronized; still differences are qualitative.',cases:[]};
 ...
 for(const [release,origin] of [['before',process.env.MADAGIN_BEFORE_ORIGIN??'http://127.0.0.1:3135'],['after',process.env.MADAGIN_REVIEW_ORIGIN??'http://127.0.0.1:3136']]) {
  ...
  for(const viewport of [{width:1440,height:900},{width:390,height:844}]) {
   ...
   for(const [id,progress] of [['opening',.03],['ridge-held',.17],['ridge-exit',.28],['valley',.34],['lake',.5],['near-canopy',.58],['waterfall',.75],['summit',.98]]) {
    ...
    const data=await page.evaluate(()=>({camera:JSON.parse(document.documentElement.dataset.madaginPublicCamera),rootedTrees:document.documentElement.dataset.madaginRootedTrees??null,surface:document.documentElement.dataset.madaginTerrainSurface?JSON.parse(document.documentElement.dataset.madaginTerrainSurface):null,canopy:document.documentElement.dataset.madaginRidgeCanopy??null,canvasCount:document.querySelectorAll('canvas').length,render:window.__MADAGIN_RIDGE_BENCHMARK_V116__?.render??null}));
