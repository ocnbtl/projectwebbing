...
const out=path.resolve(process.env.MADAGIN_BRANCH_EVIDENCE ?? 'output/releases/madagin-branch-sprays-20260910');
...
const code=(await fs.readFile(source+'/src/components/internal/rooted-trees.tsx','utf8')).replace('./compact-tree-lod','./compact-tree-lod.mjs').replace('./branch-sprays','./branch-sprays.mjs').replace('import branchSupport from '+String.fromCharCode(34)+'./branch-canopy-support.json'+String.fromCharCode(34)+';', 'const branchSupport='+await fs.readFile(source+'/src/components/internal/branch-canopy-support.json','utf8')+';');
...
const baseline='6c1db8d3555563fa692fa64ebc495545966057ee';
...
for(const distant of [false,true]){
 const parts=prepareTrees(group,group,true,distant,true),bounds=new Box3();let radius=0,baseRadius=0,triangles=0;
 ...
 cases.push({sprays:parts.filter(p=>p.geometry.userData.branchSprays).map(p=>({...p.geometry.userData.branchSprays,positionHash:hash(Buffer.from(p.geometry.getAttribute('position').array.buffer))})),distant,normalizedHeight:bounds.max.y-bounds.min.y,rootY:bounds.min.y,envelopeRadius:radius,baseRadius,triangles,passed:true});
