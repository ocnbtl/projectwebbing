...
const source=await fs.readFile('src/components/internal/lake-shore.ts','utf8');
const lake=await compile('lake-depth-current',source);
const old=await compile('lake-depth-baseline',execFileSync('git',['show','03feecc7ced24247ffe3472292f97667759fe529:src/components/internal/lake-shore.ts'],{encoding:'utf8'}));
...
glsl=glsl.replace(/vec2 coordinate=.*?;/,'const coordinate={x:(p.x+2.04)/132.4,y:(p.y+884.765)/94.6};').replace(/\bfloat /g,'let ').replaceAll('length(coordinate)','Math.hypot(coordinate.x,coordinate.y)').replace('return vec2(d,depth);','return [d,depth];');
glsl='const {sin,cos,exp,pow,min,max}=Math;const atan=Math.atan2;const clamp=(v,a,b)=>Math.max(a,Math.min(b,v));\n'+glsl+'\nexport {gpuLakeShore};';
...
for(let a=0;a<360;a+=2){let previous=-Infinity;for(let step=0;step<=140;step++){
 ...
 if(d<=1)assert.ok(bed>=previous-1e-8,'Submerged basin rises continuously toward each shore');previous=bed;
 if(d>=.82)assert.ok(Math.abs(bed-old.lakeBedLevel(x,z))<1e-8,'Shore and dry-bank authority retained');
