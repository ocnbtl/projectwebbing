...
const out=path.resolve('output/playwright/madagin-world-progress',process.env.MADAGIN_EVIDENCE_CYCLE??'riparian-ecology-release-20260909',process.env.MADAGIN_PUBLIC_EVIDENCE_LABEL??'plant-ordering-local');
await fs.mkdir(out,{recursive:true});
const report={origin,at:new Date().toISOString(),cases:[]};
const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true});
try {
 for(const {width,constrained} of [{width:1440,constrained:false},{width:390,constrained:false},{width:1440,constrained:true}])for(const held of constrained?['plant-model']:['plant-model','plant-textures']){
  ...
  const asset=held==='plant-model'?'**/world/canopy-v1/fern.glb':'**/world/canopy-v1/textures/384492d3832b1662a716a3694fadadd1e0cf3029a548f16516fad65b7084e571.webp';
