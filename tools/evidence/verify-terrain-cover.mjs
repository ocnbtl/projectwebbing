...
const origin=process.env.MADAGIN_REVIEW_ORIGIN??'http://127.0.0.1:3176';
const phase=process.env.MADAGIN_TERRAIN_PHASE??'local';
const out=path.resolve(process.env.MADAGIN_TERRAIN_EVIDENCE_ROOT??'output/releases/madagin-terrain-cover-20260909');
...
try {
  for(const width of [1440,390]) {
    ...
    const observed=await page.evaluate(()=>{
      ...
      return {surface:JSON.parse(document.documentElement.dataset.madaginTerrainSurface),fragment,linked:gl.getProgramParameter(program,gl.LINK_STATUS),draws,submittedIndices,textures,glError:gl.getError(),camera:JSON.parse(document.documentElement.dataset.madaginPublicCamera),canvasCount:document.querySelectorAll('canvas').length};
    });
