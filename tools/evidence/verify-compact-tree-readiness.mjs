...
const out=path.resolve('output/playwright/madagin-world-progress',process.env.MADAGIN_EVIDENCE_CYCLE??'compact-canopy-release-20260908',process.env.MADAGIN_PUBLIC_EVIDENCE_LABEL??'tree-ordering-local');
await fs.mkdir(out,{recursive:true});
const report={origin,at:new Date().toISOString(),cases:[]};
const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true});
try {
 for(const width of [390])for(const held of ['tree-model','tree-textures']){
  const context=await browser.newContext({viewport:{width,height:width===390?844:900}});
  await context.addInitScript(()=>{window.__MADAGIN_RIDGE_STAGES_V116__=[{at:-1,zone:'ridge',stage:0,label:'ridge-terrain-ready'},{at:-1,zone:'ridge',stage:2,label:'ridge-ecology-ready'}];});
  ...
  const asset=held==='tree-model'?'**/world/rooted-trees-v1/tree-0-far.glb':'**/world/rooted-trees-v1/textures/*.webp';
  ...
  try{
   ...
   await page.screenshot({path:path.join(out,`${width}-${held}-held.png`)});
   ...
   await page.screenshot({path:path.join(out,`${width}-${held}-ready.png`)});
   report.cases.push({width,held,pending,waiting,ready,errors,passed:true});
  }catch(error){report.cases.push({width,held,passed:false,error:String(error)});throw error;}
...
console.log(JSON.stringify({origin,cases:report.cases.map(({width,held,passed,error})=>({width,held,passed,error}))}));
