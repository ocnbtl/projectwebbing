...
const grounding=process.env.MADAGIN_BANK_GROUNDING_EVIDENCE?JSON.parse(await fs.readFile(process.env.MADAGIN_BANK_GROUNDING_EVIDENCE)):null;
if(grounding)assert.ok(grounding.passed,'Verified before/after terrain triangles required for displaced roots');
...
try{for(const width of [1440,390]){
 ...
 const groundDelta=(x,z)=>grounding?.cases.find(c=>c.compact===compact)?.errors.find(p=>p.kind==='source-bank-placements'&&Math.abs(p.x-x)<.000001&&Math.abs(p.z-z)<.000001)?.terrainDelta??0;
 const matched={rocks:expected.rocks.filter(p=>match(actual.roots.other,[p.x,p.y+groundDelta(p.x,p.z),p.z])).length,ferns:expected.ferns.filter(p=>match(actual.roots.fern,[p.x,p.y+groundDelta(p.x,p.z),p.z])).length,trees:expected.trees.filter(p=>match(actual.roots.tree,[p[2],p[3]-.025+groundDelta(p[2],p[4]),p[4]])).length};
 assert.ok(actual.assets.every(p=>!p.includes('/rooted-trees-v1/')&&!p.includes('/leaf-canopy-v1/')),'Authored crowns must not fetch retired tree models');
 assert.equal(actual.bank.version,'source-bank-1');assert.equal(actual.bank.compact,compact);assert.equal(actual.rocks.sourceBankRocks,expected.rocks.length);assert.equal(actual.trees['source-bank'].count,expected.trees.length);assert.equal(actual.trees['source-bank'].lod,'branch-crown-continuous-1');assert.equal(actual.trees['source-bank'].branchCrowns,'branch-crowns-4');assert.equal(actual.trees['source-bank'].crownShelter,'crown-shelter-1');
 ...
 for(const asset of ['/world/canopy-v1/moss-rock.glb','/world/canopy-v1/fern.glb'])assert.equal(actual.assets.filter(p=>p===asset).length,1,asset+' single cached request');
