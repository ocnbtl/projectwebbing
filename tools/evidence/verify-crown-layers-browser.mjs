...
const root=process.env.MADAGIN_CROWN_EVIDENCE_ROOT??'output/releases/madagin-crown-layers-20260910',phase=process.env.MADAGIN_CROWN_PHASE??'local',origin=process.env.MADAGIN_REVIEW_ORIGIN??'http://127.0.0.1:3186';
const expected=JSON.parse(await fs.readFile(root+'/crown-geometry-checks.json')).cases[0].sprays[0];
...
try {for(const width of [1440,390]){
 ...
 assert.equal(actual.canvas,1);assert.equal(actual.trees['bank-canopy'].crownLayers,'crown-layers-1');assert.equal(actual.trees['bank-canopy'].count,260);assert.equal(actual.error,0);assert.ok(actual.last>actual.first+.1);assert.ok(actual.draws.some(d=>d.linked&&d.indices===expected.triangles*3&&d.instances>0));
 ...
 }}finally{await browser.close();await fs.writeFile(root+'/layers-gpu-'+phase+'.json',JSON.stringify({at:new Date().toISOString(),origin,passed:cases.length===2,cases},null,2)+'\n');}
