...
try{for(const width of [1440,390]){
 ...
 assert.equal(actual.canvas,1);assert.equal(actual.optics.version,process.env.MADAGIN_EXPECTED_LAKE_OPTICS??'lake-optics-3');assert.ok(actual.last>actual.first+.1);assert.equal(actual.samples.length,1);
