...
const out=path.resolve('output/releases/madagin-compact-canopy-20260908');
const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true});
const baseline=process.env.MADAGIN_CANOPY_BASELINE==='1';
let report;
try{
 ...
 assert.equal(initial.aa,!baseline);assert.equal(initial.camera.fov,60);
 for(const zone of baseline?[]:['ridge','valley','lake']){
 ...
 const checks={representation:baseline?initial.ecology.valley.rootedMode===undefined:true,multisampleContext:initial.aa===!baseline,existingFarSourcesOnly:true,fullFlightP95:flight.p95<=33.3,fullFlightP99:flight.p99<=50,peakHeap:flight.peakHeap>0&&flight.peakHeap<=192*1048576};
