...
const root=path.resolve('output/playwright/madagin-world-progress',process.env.MADAGIN_EVIDENCE_CYCLE??'compact-canopy-release-20260908');
await fs.mkdir(root,{recursive:true});
const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true});
const report={at:new Date().toISOString(),method:'Optimized public renderer. Pause UI, hold the existing React MotionValue at fixed rail positions. Same geometry, sun, FOV (42 desktop/60 compact), positions and look targets; compact inland tree representation differs. Animation phases are not synchronized; comparisons are qualitative',cases:[]};
...
try {
 ...
 for(const after of report.cases.filter(c=>c.release==='after')) {
  ...
  for(const a of after.views) {const b=before.views.find(v=>v.id===a.id);
   ...
   assert.ok(position<1e-8);assert.ok(look<.1);assert.equal(a.camera.fov,b.camera.fov);distances.push({width:after.viewport.width,id:a.id,position,look});
  }
 }
