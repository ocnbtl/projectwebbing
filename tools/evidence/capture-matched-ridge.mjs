...
const root=path.resolve('output/playwright/madagin-world-progress/ridge-release-20260906');
...
try {
 for(const [release,origin] of [['before',process.env.MADAGIN_BEFORE_ORIGIN??'http://127.0.0.1:3135'],['after',process.env.MADAGIN_REVIEW_ORIGIN??'http://127.0.0.1:3136']]) {
  if(process.env.MADAGIN_CAPTURE_PHASE === "after" && release === "before") continue;
  for(const viewport of [{width:1440,height:900},{width:390,height:844}]) {
   ...
   const views=[];
   for(const [id,progress] of [['opening',.03],['ridge-held',.17],['ridge-exit',.28],['valley',.34],['lake',.5],['waterfall',.75],['summit',.98]]) {
    ...
    await page.waitForTimeout(id==='ridge-held'?12000:1600);
    await page.screenshot({path:path.join(out,`${id}.png`)});
    const data=await page.evaluate(()=>({camera:JSON.parse(document.documentElement.dataset.madaginPublicCamera),canopy:document.documentElement.dataset.madaginRidgeCanopy??null,canvasCount:document.querySelectorAll('canvas').length,render:window.__MADAGIN_RIDGE_BENCHMARK_V116__?.render??null}));
