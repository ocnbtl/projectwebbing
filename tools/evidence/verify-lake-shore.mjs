...
const out=path.resolve('output/releases/madagin-lake-shore-20260908');
...
}).join('\n')+'\nexport {createNativeRidgeSurface,createRidgeErosionTerrainGeometry,geometrySurfaceForMerge,v116RiverCenter,ridgeChannelWidth,createRidgeHeadwaterGeometry,createIntegratedRiverGeometry,outflowTerrainSampler,createWaterfallOutflowGeometry,createIntegratedWatershedTerrainGeometry,createExactBoundaryTerrainSeamBridge,extractTerrainSeamSamples,createIntegratedLakeGeometry,createWaterMaterial};';
const runtime=await compile('fixture-runtime',fixture);
const baseline=await import(pathToFileURL(path.resolve('output/releases/madagin-headwater-20260908/fixture-runtime.mjs')).href);
...
for(const compact of [false,true]){
 ...
 assert.equal(hashArray(valley.getAttribute('position').array),hashArray(oldValley.getAttribute('position').array),'Valley terrain positions retained exactly');
 assert.equal(hashArray(valley.index.array),hashArray(oldValley.index.array),'Valley topology retained exactly');
 oldValley.dispose();
...
try {
 ...
 for(let a=0;a<360;a+=2)for(const radius of [.3,.8,.96,1,1.025,1.05,1.075,1.12]){
  const angle=a*Math.PI/180,scale=shore.lakeBoundaryScale(angle);
  points.push([shore.LAKE_CENTER.x+Math.cos(angle)*shore.LAKE_RADIUS.x*scale*radius,shore.LAKE_CENTER.z+Math.sin(angle)*shore.LAKE_RADIUS.z*scale*radius]);
 }
 ...
 },{points,source:shore.LAKE_SHORE_GLSL});
 const maxDepthError=Math.max(...points.map((p,i)=>Math.abs((shore.LAKE_WATER_LEVEL-shore.lakeBedLevel(...p))-gpu[i][1])));
 const maxBoundaryError=Math.max(...points.map((p,i)=>Math.abs(shore.lakeBoundaryDistance(...p)-gpu[i][0])));
