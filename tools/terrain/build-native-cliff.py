...
lon,lat=-159.661,22.170;dx=sx*111320*np.cos(np.radians(22.174));dz=sy*111320
...
data={'version':'native-cliff-2','sourceSha256':sha,'source':'NOAA CUDEM Hawaii 2021','sourceUrl':'https://chs.coast.noaa.gov/htdata/raster2/elevation/NCEI_ninth_Topobathy_Hawaii_9428/tiles/ncei19_n22x25_w159x75_2021v1.tif','sourcePixelWindow':[ix,iz,nx+1,nz+1],'sourceCenter':[lon,lat],'sceneYawDegrees':90,'sourceGridMeters':[float(dz),float(dx)],'rows':nx+1,'columns':nz+1,'x0':-300,'z0':-250,'width':float(nz*dz),'depth':float(nx*dx),'verticalTranslation':0,'collarMeters':40,'sourceRange':[float(h.min()),float(h.max())],'precisionMeters':.01,'elevations':[round(float(v),2) for v in np.rot90(h,-1).ravel()]}
