...
# Retain the scene tangent-plane longitude scale at 22.174 N for stable bounds.
lon,lat=-159.657,22.172;dx=sx*111320*np.cos(np.radians(22.174));dz=sy*111320
...
data={'version':'native-valley-2','sourceSha256':sha,'source':'NOAA CUDEM Hawaii 2021','sourceUrl':'https://chs.coast.noaa.gov/htdata/raster2/elevation/NCEI_ninth_Topobathy_Hawaii_9428/tiles/ncei19_n22x25_w159x75_2021v1.tif','sourcePixelWindow':[ix,iz,nx+1,nz+1],'sourceCenter':[lon,lat],'sceneYawDegrees':90,'sourceGridMeters':[float(dz),float(dx)],'rows':nx+1,'columns':nz+1,'x0':-350,'z0':-890,'width':float(nz*dz),'depth':float(nx*dx),'verticalTranslation':0,'collarMeters':55,'sourceRange':[float(h.min()),float(h.max())],'precisionMeters':.01,'elevations':[round(float(v),2) for v in np.rot90(h,-1).ravel()]}
out=ROOT/'src/components/internal/native-valley-field.json';out.write_text(json.dumps(data,separators=(',',':'))+'\n',encoding='utf-8',newline='\n')
proof={k:v for k,v in data.items() if k!='elevations'};proof.update({'rights':'NOAA public domain in the United States; CIRES CUDEM DOI 10.25921/ds9v-ky35','accuracy':'Nominal 1/9 arc-second grid, mixed/interpolated sources; not uniform 3 m survey accuracy','transform':'Native source grid and metres retained. Geographic window rotated 90 degrees about vertical and translated into an authored scene. The 55 m outer collar and shared authored lake exclusion blend to retained terrain. The lake footprint through normalized boundary distance 1.45 is unchanged; native relief fades in by 1.85. The coastal seam and source-normal buffer stay unchanged through x=-285, blending to full relief by x=-245. No residual extraction, relief exaggeration or geographic continuity claim for the whole world.','fieldBytes':out.stat().st_size,'fieldSha256':hashlib.sha256(out.read_bytes()).hexdigest(),'quantizationMaxErrorMeters':float(np.max(np.abs(h-np.round(h,2))))})
p=ROOT/'output/releases/madagin-valley-relief-20260910/source-field.json';p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps(proof,indent=2)+'\n',encoding='utf-8');print(json.dumps({k:v for k,v in proof.items() if k in ['sourceRange','fieldBytes','sourceGridMeters','width','depth','quantizationMaxErrorMeters']}))
