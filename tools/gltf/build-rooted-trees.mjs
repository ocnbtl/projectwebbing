...
const out=path.resolve('public/world/rooted-trees-v1');await fs.mkdir(path.join(out,'textures'),{recursive:true});
...
const report={version:1,license:'CC0-1.0',classification:'Generic licensed tree architecture, not a survey of Hawaiian species.',policy:"Sources remain untouched. Near derivatives retain every connected source leaf and simplify each separately; far derivatives retain one in six leaves across the whole 3D crown and expand those leaves 2.05x locally. Bark/branches use Meshopt relative error .006 near/.012 far. Position/normal/UV quantization is 14/10/12 bits. World transforms are decoded before rooting and normalization. Shared 512px WebP maps retain data at quality95/color90; black leaf-atlas margins acquire alpha and transparent RGB is filled from the opaque source mean to avoid dark mips. Runtime alpha cutoff .12 with MSAA coverage. Artistic height/width variation is shared by all parts. These are generic trees, not native species measurements.",assets:[]};
for(const [variant,name] of ['island_tree_02','tree_small_02'].entries()) {
 ...
 for(const lod of ['near','far']) {
  ...
  for(const tex of doc.getRoot().listTextures()) {
   ...
   tex.setImage(bytes).setMimeType('image/webp').setURI(`textures/${sha(bytes)}.webp`);
  }
  ...
  const textures=[];
  for(const image of pack.json.images){const b=Buffer.from(pack.resources[image.uri]);await fs.writeFile(path.join(out,image.uri),b);textures.push({path:image.uri,bytes:b.length,sha256:sha(b)});}
