...
const out=path.resolve(process.env.MADAGIN_ASSET_OUTPUT??'public/world/branch-canopy-v1');await fs.mkdir(path.join(out,'textures'),{recursive:true});
...
function reduceLeaves(p,lod) {
 ...
 for(const group of groups.values()){
  const keep=lod==='near'||leafIndex%6===0;leafIndex++;if(!keep)continue;
 ...
 return {sourceLeaves:groups.size,retainedLeaves:lod==='near'?groups.size:Math.ceil(groups.size/6),farLeafExpansion:lod==='near'?1:2.05,maxLeafRelativeError:maxLeafError};
}
const report={version:'branch-canopy-source-1',license:'CC0-1.0',classification:'Existing generic Poly Haven coastal tree, not a measured Hawaiian species.',policy:'Preserve original source. One in six whole source leaves at 2.05x local expansion; same existing simplification and 14/10/12-bit position/normal/UV policy. Dense connected source branching replaces one near-bank variant only. No lighting-normal trial is included.',assets:[]};
for(const [variant,name] of ['island_tree_01'].entries()) {
 ...
 for(const lod of ['far']) {
  ...
  for(const tex of doc.getRoot().listTextures()) {
   ...
   const file=sha(bytes)+'.webp';let shared=false;try{shared=bytes.equals(await fs.readFile('public/world/rooted-trees-v1/textures/'+file));}catch{}const uri=shared?'../rooted-trees-v1/textures/'+file:'textures/'+file;if(!shared)await fs.writeFile(path.join(out,uri),bytes);tex.setImage(bytes).setMimeType('image/webp').setURI(uri);
  }
  ...
  const bytes=Buffer.concat([h,jh,j,bh,b]),file=`island-tree-01-${lod}.glb`;await fs.writeFile(path.join(out,file),bytes);
